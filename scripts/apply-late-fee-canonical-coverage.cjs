...
const waiverAwareDecisionCoverage = `    if (!charge || charge.status === "PAID" || charge.status === "VOID") {
      throw new Error("Charge is not open for late payment fee management.");
    }
...
if (!source.includes(newDecisionCoverage)) {
  if (source.includes(waiverAwareDecisionCoverage)) {
    source = source.replace(waiverAwareDecisionCoverage, newDecisionCoverage);
  } else if (source.includes(oldDecisionCoverage)) {
