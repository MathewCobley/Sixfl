...
function patchPage() {
  ...
  if (!source.includes("Apply league filter")) {
    ...
    const clearHref = hasExcludeFilters
      ? `buildHref({ q: selectedQuery, type: selectedType, status: selectedStatus, area: selectedArea, night: selectedNight, excludeType: excludedType, excludeStatus: excludedStatus })`
      : `buildHref({ q: selectedQuery, type: selectedType, status: selectedStatus, area: selectedArea, night: selectedNight })`;
