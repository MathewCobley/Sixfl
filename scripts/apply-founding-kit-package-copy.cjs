...
replaceAllText(leadDetail, [
  ['label="Free kit"', 'label="£90 kit package"'],
]);
...
replaceAllText(captainForm, [
  ["Submit all nine kits", "Submit £90 kit package"],
]);
...
const publicCopy = [read(registerPage), read(registerActions)].join("\n");
if (/founding teams free kit offer|Free kit interest:/i.test(publicCopy)) {
  throw new Error("Old public free-kit wording remains after the pricing copy update.");
}
