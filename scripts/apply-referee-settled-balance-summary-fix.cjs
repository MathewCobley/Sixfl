...
// Referee balance timing and payment presentation are now owned by one shared
// source of truth in referee-nights.ts. Prebuild verifies that both the portal
// and the shared helpers still enforce those safeguards without rewriting React.
const pageMarkers = [
