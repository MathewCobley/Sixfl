...
source = source.replace(
  ...
  'import { listAdminKitPaymentActivity } from "@/lib/kits/admin-payment-activity";',
);

if (!source.includes("listAdminKitPaymentActivity")) {
  replaceOnce(
...
source = source.replace(
  ...
  "const kitPaymentActivity = await listAdminKitPaymentActivity();",
);

if (!source.includes("const kitPaymentActivity = await listAdminKitPaymentActivity()")) {
  replaceOnce(
  ...
  replaceOnce(
    ...
    `      <section className="rounded-3xl border border-amber-400/20 bg-amber-500/[0.06] p-5 sm:p-6">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <div className="text-[10px] font-semibold uppercase tracking-[0.2em] text-amber-200/65">Coming soon</div>
            <h2 className="mt-2 text-xl font-semibold text-white">Started, not submitted</h2>
            <p className="mt-2 text-sm text-white/55">Teams appear here as soon as they save a draft or start kit payments. Kit payments count as started even before a draft is saved.</p>
          </div>
          <div className="text-3xl font-semibold text-amber-100">{comingSoonKitTeams.length}</div>
        </div>

        {comingSoonKitTeams.length === 0 ? (
          <div className="mt-5 rounded-2xl border border-dashed border-white/10 bg-black/15 p-5 text-sm text-white/45">No teams have a draft or active kit payment activity yet.</div>
        ) : (
          <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {comingSoonKitTeams.map((item) => {
              ...
              const league = item.leagueName ? (item.leagueSeason ? \`${'${item.leagueName}'} · ${'${item.leagueSeason}'}\` : item.leagueName) : "No league assigned";
              return (
                <div key={item.teamId} className="rounded-2xl border border-amber-400/15 bg-black/20 p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0"><div className="truncate font-semibold text-white">{item.teamName}</div><div className="mt-1 truncate text-xs text-white/45">{league}</div></div>
                    <div className="flex flex-wrap justify-end gap-1.5">
                      {item.order ? <span className="shrink-0 rounded-full border border-amber-400/25 bg-amber-500/10 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-amber-100">Draft saved</span> : <span className="shrink-0 rounded-full border border-sky-400/25 bg-sky-500/10 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-sky-100">Payment started</span>}
                    </div>
                  </div>
                  {item.order ? <div className="mt-3 text-sm text-white/65">Kit: <span className="font-semibold text-white">{item.order.design?.code ?? "Not chosen"}</span></div> : null}
                  <div className="mt-3 flex flex-wrap gap-2">
                    {paid > 0 ? <span className="rounded-full border border-emerald-400/20 bg-emerald-500/10 px-2.5 py-1 text-xs font-semibold text-emerald-100">{paid} kit{paid === 1 ? "" : "s"} paid</span> : null}
                    {pending > 0 ? <span className="rounded-full border border-amber-400/20 bg-amber-500/10 px-2.5 py-1 text-xs font-semibold text-amber-100">{pending} awaiting payment</span> : null}
                  </div>
                  <div className="mt-2 text-xs text-white/40">{item.order ? \`${'${item.order.items.length}'} of ${'${item.order.kitQuantity}'} kit rows saved · Last changed ${'${formatDate(item.activityAt)}'}\` : \`Payment activity · Last changed ${'${formatDate(item.activityAt)}'}\`}</div>
                  <Link href={\`/captain/team/${'${item.teamId}'}/kit\`} className="mt-4 inline-flex min-h-9 items-center justify-center rounded-xl border border-white/10 bg-white/[0.05] px-3 text-xs font-semibold text-white/70 transition hover:bg-white/[0.08]">Open kit page</Link>
                </div>
              );
            })}
          </div>
