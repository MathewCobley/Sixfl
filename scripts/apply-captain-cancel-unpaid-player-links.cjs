...
// The older detailed ledger used a separate summary box for open links. The
// simplified ledger intentionally replaces that with one concise warning and one
// action. Do not try to recreate the old duplicate controls when the simplified
// ledger has already been applied earlier in the build chain.
const simplifiedLedgerApplied =
  source.includes("Player link still open — not needed") &&
  source.includes("Close unpaid player");
...
const hasUsableCancellationControl =
  source.includes("Cancel all unpaid player links") ||
  (source.includes("Player link still open — not needed") &&
    source.includes("Close unpaid player"));

if (
  !hasUsableCancellationControl ||
  !source.includes("cancelledAt: new Date()") ||
  !source.includes("paymentToken: null") ||
  (!simplifiedLedgerApplied && !source.includes("Each payment will reduce that balance")) ||
  source.includes(
    '    !entry.fixtureId ||\n    entry.displayStatus !== "PAID" ||\n    entry.playerOpenPence <= 0',
  )
) {
...
console.log(
  simplifiedLedgerApplied
    ? "Captain unpaid player-link cancellation remains enabled in the simplified payment ledger."
