...
patchFile(captainLayoutPath, [
  {
    label: "captain team free-kit lookup",
    before: [
      "  if (!team) {",
      "    notFound();",
      "  }",
    ...
    after: [
      ...
      "  }",
      "",
      "  const freeKitRows = await prisma.$queryRaw<Array<{ wantsFreeKit: boolean }>>`",
      '    SELECT COALESCE("wantsFreeKit", false) AS "wantsFreeKit"',
      '    FROM "Team"',
      '    WHERE "id" = ${teamid}',
      "    LIMIT 1",
      "  `;",
      "  const wantsFreeKit = Boolean(freeKitRows[0]?.wantsFreeKit);",
      "",
      "  const displayCompetition = team.league?.competition ?? null;",
    ].join("\n"),
  },
...
patchFile(kitPagePath, [
  {
    ...
    after: [
      ...
      "  `;",
      "",
      "  if (!freeKitRows[0]?.wantsFreeKit) notFound();",
      "",
      "  const [allDesigns, order] = await Promise.all([",
    ].join("\n"),
  },
