...
export function monthlyCandidatePayload(row: MonthlyCandidate) {
  return {
    ...
    clipAssetId: row.clipAssetId,
    clipNumber: row.clipNumber == null ? null : Number(row.clipNumber),
    ...
    clipUrl: row.clipAssetId ? `/api/goal-of-month/clips/${row.id}` : null,
    thumbnailUrl: row.clipAssetId ? `/api/goal-of-month/clips/${row.id}/thumbnail` : null,
...
export async function getMonthlyCandidates(key: string, limit = 300, db: Db = prisma): Promise<MonthlyCandidate[]> {
  ...
  return db.$queryRaw<MonthlyCandidate[]>(Prisma.sql`
    SELECT c."id", c."fixtureId", c."teamId", c."monthKey", c."goalNumber",
      c."clipAssetId", clip."clipNumber"::int AS "clipNumber", c."scorerName", c."createdAt",
    ...
    WHERE c."monthKey" = ${key} AND c."status" = 'ACTIVE'
      ...
      AND (
        (c."clipAssetId" IS NOT NULL AND clip."id" IS NOT NULL)
        OR (c."clipAssetId" IS NULL AND f."sixflTvRecorded" = TRUE AND COALESCE(f."sixflTvUrl", '') <> '')
      )
      AND c."teamId" IN (f."homeTeamId", f."awayTeamId")
    GROUP BY c."id", clip."clipNumber", t."name", t."logoUrl", f."homeTeamId", away."name", home."name", l."name", f."kickoffAt", f."sixflTvUrl"
...
export async function getMonthlyFixtures(key: string, db: Db = prisma, fixtureId?: string): Promise<MonthlyFixture[]> {
  const period = monthlyPeriod(key);
  const fixtures = await db.$queryRaw<Array<Omit<MonthlyFixture, "clips">>>(Prisma.sql`
    ...
    WHERE f."kickoffAt" >= ${period.startsAt} AND f."kickoffAt" < ${period.endsAt}
      ...
      AND (
        EXISTS (
          SELECT 1 FROM "SixflTvFootageAsset" asset
          WHERE asset."fixtureId" = f."id"
            AND asset."kind" = 'CLIP'
            AND asset."state" = 'READY'
            AND asset."clipNumber" IS NOT NULL
        )
        OR (f."sixflTvRecorded" = TRUE AND COALESCE(f."sixflTvUrl", '') <> '')
      )
  ...
  const clips = await db.$queryRaw<MonthlyClip[]>(Prisma.sql`
    SELECT "id" AS "assetId", "fixtureId", "clipNumber"::int AS "clipNumber", "filename"
    FROM "SixflTvFootageAsset"
    WHERE "fixtureId" IN (${Prisma.join(fixtureIds)})
      AND "kind" = 'CLIP'
      AND "state" = 'READY'
      AND "clipNumber" IS NOT NULL
    ORDER BY "fixtureId", "clipNumber"
  `);
  const byFixture = new Map<string, MonthlyClip[]>();
  for (const clip of clips) {
    const rows = byFixture.get(clip.fixtureId) ?? [];
    rows.push(clip);
    byFixture.set(clip.fixtureId, rows);
  }
  return fixtures.map(row => ({ ...row, clips: byFixture.get(row.id) ?? [] }));
}
    ...
    return {
      ...
      fixtures: fixtures.map(f => ({
        ...
        clips: f.clips.map(clip => ({
          ...clip,
          clipUrl: `/api/goal-of-month/fixtures/${f.id}/clips/${clip.assetId}`,
          thumbnailUrl: `/api/goal-of-month/fixtures/${f.id}/clips/${clip.assetId}/thumbnail`,
        })),
      })),
...
export async function nominateMonthlyGoal(input: {
  ...
  return prisma.$transaction(async tx => {
    ...
    const identity = input.clipAssetId
      ? `clip:${input.clipAssetId}`
      : `goal:${input.fixtureId}:${input.goalNumber ?? ""}`;
    await tx.$executeRaw(Prisma.sql`SELECT pg_advisory_xact_lock(hashtextextended(${'monthly-goal:' + identity}, 0))`);
    ...
    if (!fixture) throw new GoalAwardError("Choose a completed, published SIXFL fixture with available footage.", 400);
    ...
    let clipAssetId: string | null = null;

    if (input.clipAssetId) {
      const clip = fixture.clips.find(row => row.assetId === input.clipAssetId);
      if (!clip) throw new GoalAwardError("Choose one of the saved highlight clips from that match.", 400);
      ...
      if (!Number.isInteger(input.goalNumber) || Number(input.goalNumber) < 1 || Number(input.goalNumber) > goals)
        throw new GoalAwardError("Choose a valid goal number from that match.", 400);
      goalNumber = Number(input.goalNumber);
      [existing] = await tx.$queryRaw<Array<{ id: string; teamId: string; status: string }>>(Prisma.sql`
        SELECT "id", "teamId", "status"
        FROM "GoalOfMonthCandidate"
        WHERE "fixtureId" = ${input.fixtureId} AND "goalNumber" = ${goalNumber}
        LIMIT 1
      `);
    }
    if (existing && (existing.status !== 'ACTIVE' || existing.teamId !== input.scoringTeamId))
      throw new GoalAwardError("That clip was removed or has a different scoring team recorded. Ask SIXFL to review it.");
