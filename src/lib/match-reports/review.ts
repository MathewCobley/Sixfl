...
export type MatchReportWarning = { fixtureId: string; resultId: string; teamId: string; teamName: string; homeName: string; awayName: string; kickoffAt: Date; goalsRecorded: number; assistsRecorded: number; goalsExpected: number };
...
export async function getMatchReportWarnings(fixtureId?: string, fixtureIds?: string[]) {
  ...
  return prisma.$queryRaw<MatchReportWarning[]>(Prisma.sql`
    SELECT f."id" AS "fixtureId", r."id" AS "resultId", m."teamId", t."name" AS "teamName",
      ...
      m."goalsRecorded", totals.assists AS "assistsRecorded", scores.expected AS "goalsExpected"
    ...
    WHERE (m."goalsRecorded" > scores.expected OR totals.assists > scores.expected)
