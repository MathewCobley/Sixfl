...
// SIXFL's exported Prisma client is extended with fixture safety guards. Prisma's
// generated TransactionClient type does not describe that extended transaction
// callback correctly, so this small internal structural escape hatch is kept
// local to this repair module.
type RepairTransaction = any;
...
function parsedInput(input: SharedEmailRepairInput) {
  return {
...
async function getExactNamedUserOnEmail(email: string, normalisedName: string) {
  const rows = await prisma.$queryRaw<UserRow[]>(Prisma.sql`
    SELECT
      player_user."id",
      player_user."name",
    ...
    LEFT JOIN "Team" team ON team."id" = member."teamId"
    WHERE LOWER(BTRIM(COALESCE(player_user."email", ''))) = ${email}
      AND LOWER(REGEXP_REPLACE(BTRIM(COALESCE(player_user."name", '')), '[[:space:]]+', ' ', 'g')) = ${normalisedName}
    GROUP BY player_user."id", player_user."name", player_user."email"
    LIMIT 1
  `);
  return rows[0] ?? null;
...
export async function getSharedEmailRepairPreview(
  ...
  const [sharedEmailUser, separateUserOnSharedEmail, newEmailUser, leads, prospects, counts] =
    await Promise.all([
      ...
      getExactNamedUserOnEmail(parsed.sharedEmail, parsed.separateNameNormalised),
      getUserByEmail(parsed.newEmail),
      prisma.$queryRaw<LeadRow[]>(Prisma.sql`
        SELECT
          lead."id", lead."contactName", lead."email", lead."phone",
  ...
  if (leads.length === 0 && prospects.length === 0 && !separateUserOnSharedEmail) {
    warnings.push(
      "No exact-name lead/prospect currently uses the old email. That is acceptable when the person has already moved to a new lead email: the repair can still clean stale player-source communication metadata without changing football history.",
    );
  }
  ...
  return {
    ...
    recipients: {
      ...
      playerSources: Number(summary.playerSources),
      leadSources: Number(summary.leadSources),
      otherSources: Number(summary.otherSources),
    },
    blockers,
    warnings,
...
async function ensureAuditTable(tx: RepairTransaction) {
...
async function loadAuthoritativePlayerRecipientDetails(
  tx: RepairTransaction,
  sharedEmail: string,
): Promise<PlayerRecipientTruthRow[]> {
  const fees = await tx.$queryRaw<PlayerRecipientTruthRow[]>(Prisma.sql`
    SELECT
  ...
  const selections = await tx.$queryRaw<PlayerRecipientTruthRow[]>(Prisma.sql`
    SELECT
      recipient."id" AS "recipientId",
  ...
  const prospects = await tx.$queryRaw<PlayerRecipientTruthRow[]>(Prisma.sql`
    SELECT
      recipient."id" AS "recipientId",
  ...
  return [...fees, ...selections, ...prospects];
}
...
export async function applySharedEmailRepair(input: {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (prospectIds.length) {
      const exists = await tx.$queryRaw<Array<{ exists: boolean }>>(Prisma.sql`
    ...
    // Crucially, stale shared-email player notifications are repaired from the
    // football record itself. We never infer ownership from the shared email.
    const authoritativeRows = await loadAuthoritativePlayerRecipientDetails(tx, sharedEmail);
