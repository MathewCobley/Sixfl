      ...
      {feed.items.length ? feed.items.map(news => {
        ...
        return <Link key={news.id}
          ...
          <div className="p-4">
              ...
              <div className="mt-3 rounded-xl border border-emerald-300/15 bg-emerald-500/[0.07] px-3 py-3">
                ...
                <div className="mt-1.5 space-y-2">
                  {teamMatches.map((match) => (
                    <div key={match.fixtureId} className="flex items-center justify-between gap-3">
                      <h2 className="min-w-0 text-[15px] font-black leading-5 text-white">
                        {match.teamA} <span className="text-white/35">vs</span> {match.teamB}
                      </h2>
            ...
            <p className="mt-3 text-[10px] font-bold uppercase tracking-[0.12em] text-white/35">From the full matchweek report</p>
            <p className="mt-1 line-clamp-2 text-sm font-bold leading-5 text-white/75">{news.article.title}</p>
            <span className="mt-3 inline-flex min-h-8 items-center text-xs font-bold text-emerald-300">Read matchweek report →</span>
          </div>
        </Link>;
