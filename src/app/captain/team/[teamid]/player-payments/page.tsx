...
export default async function CaptainPlayerPaymentsPage({ params, searchParams }: Props) {
  ...
  const openCharges = allCharges.filter((charge) => charge.status !== "PAID" && charge.status !== "VOID");
  ...
  const teamFeeStillToCoverPence = selectedTeamChargePaid ? 0 : selectedTeamCharge?.outstandingPence ?? 0;
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        {[
          { label: "Team fee", value: formatMoney(teamFeePence), text: selectedTeamChargePaid ? "Team charge already paid." : selectedTeamCharge ? "Open team charge in ledger." : "No open charge in ledger.", tone: selectedTeamChargePaid || !selectedTeamCharge ? "emerald" as Tone : "white" as Tone },
          ...
          { label: "Team fee still to cover", value: formatMoney(teamFeeStillToCoverPence), text: selectedTeamChargePaid || !selectedTeamCharge ? "No action needed." : "Mirrors open team charges.", tone: teamFeeStillToCoverPence > 0 ? "red" as Tone : "emerald" as Tone },
      ...
      <section className={`rounded-3xl border p-5 text-sm ${getToneClasses(selectedTeamChargePaid || !selectedTeamCharge ? "emerald" : allocation.tone)}`}>
