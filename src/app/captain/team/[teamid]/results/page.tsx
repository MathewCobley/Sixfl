...
function getContributionForPlayer(
  contributions: ContributionRow[],
  player: MatchPlayerOption,
) {
  return (
    contributions.find((item) => item.teamMemberId === player.id) ??
    contributions.find(
      (item) => normalisePlayerName(item.name) === normalisePlayerName(player.name),
    )
  );
}
...
function getFriendlyErrorMessage(error: unknown) {
  ...
  if (error.message.includes("scorer goals cannot exceed")) {
  ...
  if (error.message.includes("Dispute reason")) {
    return "Please enter a short reason for the dispute.";
  }
  if (error.message.includes("An active dispute")) {
  ...
  try {
    ...
    for (const player of players) {
      ...
      if (!Number.isInteger(goals) || goals < 0) {
        throw new Error("Goals must be whole numbers.");
      }
      if (!Number.isInteger(assists) || assists < 0) {
        throw new Error("Assists must be whole numbers.");
      }
      if (rating !== null && !ratingIsValid(rating)) {
      ...
      const played =
        formData.get(`played_${player.id}`) === "on" ||
        rating !== null ||
        goals + assists > 0 ||
        playerOfMatchTeamMemberId === player.id;
    ...
    await Promise.all([
      prisma.matchResultTeamMeta.upsert({
        where: {
...
async function createResultDispute(formData: FormData) {
  ...
  try {
    const result = await prisma.matchResult.findUnique({
      where: { id: resultId },
      include: { fixture: { select: { homeTeamId: true, awayTeamId: true } } },
    });
...
export default async function CaptainResultsPage({
  ...
  const resultIds = fixtures.flatMap((fixture) =>
    fixture.result ? [fixture.result.id] : [],
  );
  const performances = await getMatchPerformances(teamid, resultIds);
  const performancesByResult = new Map<string, MatchPerformance[]>();

  for (const performance of performances) {
    const rows = performancesByResult.get(performance.matchResultId) ?? [];
    rows.push(performance);
    performancesByResult.set(performance.matchResultId, rows);
  }
  ...
  const rows = fixtures
    .map((fixture) => {
      ...
      return {
        ...
        needsPom,
        needsAppearances,
        latestDispute: fixture.result!.disputes[0] ?? null,
        disputeDeadlineAt: getResultDisputeDeadline(fixture.result!.enteredAt),
        isDisputeWindowOpenNow: isResultDisputeWindowOpen(
    ...
    .filter((row) => {
      ...
      return [
        ...
        ...row.contributions.map((item) => item.name),
        ...row.matchPlayers.map((item) => item.name),
      ]
        .join(" ")
        .toLowerCase()
        .includes(query);
    ...
    <div className="space-y-6">
      <section className="overflow-hidden rounded-3xl border border-emerald-400/15 bg-[radial-gradient(circle_at_top_left,rgba(16,185,129,0.16),transparent_34%),linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.03))] shadow-[0_24px_80px_rgba(0,0,0,0.3)]">
        <div className="px-6 py-6 lg:px-8 lg:py-8">
          ...
          <p className="mt-3 max-w-3xl text-sm text-white/70 sm:text-base">
            Record who played, player ratings, scorers, optional assists and Player of the Match together in one place. The official score stays locked.
          </p>
        </div>
      </section>
      ...
      <div className="space-y-4">
        ...
        {rows.map((row) => {
          ...
          const recordedGoalTotal = row.contributions.reduce(
            (sum, item) => sum + item.goals,
            0,
          );
          const recordedAssistTotal = row.contributions.reduce(
          ...
          const ratings = row.matchPerformances.flatMap((item) =>
            item.rating === null ? [] : [Number(item.rating)],
          );
          const matchAverageRating =
            ratings.length > 0
                ...
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <span className="rounded-full border border-white/10 px-3 py-1 text-sm text-white/70">
                      {isComplete ? "Complete" : "Needs completion"}
                    </span>
                    {row.latestDispute ? (
                      ...
                      <div>
                        ...
                        <div className="mt-2 flex flex-wrap gap-2">
                          <span className="rounded-full border border-sky-400/20 bg-sky-500/10 px-3 py-1 text-sm text-sky-100">
                            {row.matchPerformances.length} played
                          </span>
                          <span className="rounded-full border border-sky-400/20 bg-sky-500/10 px-3 py-1 text-sm text-sky-100">
                            {ratings.length} rated
                          </span>
                          <span className="rounded-full border border-sky-400/20 bg-sky-500/10 px-3 py-1 text-sm text-sky-100">
                            Average {matchAverageRating === null ? "—" : `${matchAverageRating.toFixed(1)}/10`}
                          </span>
                        </div>
                      </div>
