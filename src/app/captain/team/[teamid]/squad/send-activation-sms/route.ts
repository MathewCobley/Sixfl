...
export async function POST(
  ...
  if (!prospect.phone?.trim()) {
    return NextResponse.redirect(getSquadRedirectUrl(request, teamid, "?error=This%20player%20does%20not%20have%20a%20phone%20number."));
  }
  ...
  return NextResponse.redirect(getSquadRedirectUrl(request, teamid, "?saved=activation-sms-sent"));
}
