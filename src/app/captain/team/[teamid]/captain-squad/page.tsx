...
export default async function CaptainSquadViewPage({
  ...
  const regularPlayerCount = team.members.filter((member) => member.role === "PLAYER").length;
  const backupPlayerCount = team.members.filter((member) => member.role === "BACKUP_PLAYER").length;
  const linkedSquadCount = team.members.length;
  const pendingAccountCount = team.prospects.length;
  const totalSquadCount = linkedSquadCount + pendingAccountCount;
    ...
    <div className="space-y-8">
        ...
        <div className="grid gap-8 px-6 py-6 lg:grid-cols-[1.15fr_0.85fr] lg:px-8 lg:py-8">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-2">
            {[
              ...
              { label: "Players", value: regularPlayerCount, copy: "Regular linked player roles.", tone: "white" },
              { label: "Backups", value: backupPlayerCount, copy: "Backup player roles.", tone: "sky" },
            ].map((metric) => {
              const toneClasses =
                metric.tone === "amber"
      ...
      {captainCount === 0 ? (
        <section className="rounded-2xl border border-amber-400/20 bg-amber-500/10 p-4 text-sm text-amber-100">
          No captain role is currently shown in the squad list. SIXFL admin can update this in the admin squad console.
        </section>
      ) : null}
                ...
                {team.prospects.map((prospect) => {
                  ...
                  return (
                    <div key={prospect.id} className="rounded-2xl border border-white/10 bg-black/20 px-4 py-3">
                      ...
                      <div className="mt-1 text-xs text-white/45">
                        Added to active squad {formatUkDateTime(prospect.updatedAt)}
                      </div>
                    </div>
                  );
                })}
