// ========================================
// File: src/app/captain/team/[teamid]/availability/WhatsAppAvailabilityBadges.tsx
// ========================================
...
type WhatsAppAvailabilityBadgesProps = {
  entries: WhatsAppBadgeEntry[];
};

function makeBadge() {
  const badge = document.createElement("span");
  badge.setAttribute("data-sixfl-whatsapp-availability-badge", "true");
...
function findAvailabilityRowFromEmail(email: string) {
  const emailNodes = getTextNodesContaining(email);

  for (const node of emailNodes) {
    const parentElement = node.parentElement;
    const row = parentElement?.closest("div.grid.gap-4.px-6.py-5");
...
function addWhatsAppBadges(entries: WhatsAppBadgeEntry[]) {
  for (const entry of entries) {
    ...
    const row = findAvailabilityRowFromEmail(entry.email.trim());
    if (!row || row.querySelector("[data-sixfl-whatsapp-availability-badge='true']")) continue;
...
export default function WhatsAppAvailabilityBadges({
  entries,
}: WhatsAppAvailabilityBadgesProps) {
  useEffect(() => {
