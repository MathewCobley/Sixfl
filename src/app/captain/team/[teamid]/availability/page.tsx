...
export default async function CaptainAvailabilityPage({
  ...
  const [teamMemberProfilesByMemberId, smsDispatches] = await Promise.all([
      ...
      ? prisma.notificationDispatch.findMany({
          where: {
            ...
            OR: fixtureIds.flatMap((fixtureId) =>
              teamMemberIds.map((teamMemberId) => ({ sourceId: getSmsSourceId({ fixtureId, teamMemberId }) })),
            ),
          },
          select: {
            sourceId: true,
  ...
  const totalAvailable = fixtures.reduce(
    (sum, fixture) => sum + fixture.availabilities.filter((item) => item.response === "AVAILABLE").length,
    0,
  );
  const totalMaybe = fixtures.reduce(
    (sum, fixture) => sum + fixture.availabilities.filter((item) => item.response === "MAYBE").length,
    0,
  );
  const totalUnavailable = fixtures.reduce(
    (sum, fixture) => sum + fixture.availabilities.filter((item) => item.response === "UNAVAILABLE").length,
    0,
  );
  const totalResponded = fixtures.reduce(
    (sum, fixture) => sum + fixture.availabilities.filter((item) => item.response !== "NO_RESPONSE").length,
    0,
  );
    ...
    <div className="space-y-8">
      <section className="overflow-hidden rounded-3xl border border-emerald-400/15 bg-[radial-gradient(circle_at_top_left,rgba(16,185,129,0.16),transparent_34%),linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.03))] shadow-[0_24px_80px_rgba(0,0,0,0.3)]">
        <div className="grid gap-8 px-6 py-6 lg:grid-cols-[1.1fr_0.9fr] lg:px-8 lg:py-8">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-emerald-300/80">Matchday planning</p>
            <h1 className="mt-3 text-3xl font-semibold tracking-tight text-white sm:text-4xl">Availability</h1>
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs font-medium text-white/75">
                {team.league?.name ?? "No league assigned"}{team.league?.season ? ` - ${team.league.season}` : ""}
              </span>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <Link href={`/captain/team/${teamid}/availability/history`} className="inline-flex items-center rounded-full border border-emerald-400/30 bg-emerald-500/15 px-5 py-3 text-sm font-medium text-emerald-50 transition hover:bg-emerald-500/20">
                View availability history
              </Link>
              <Link href={`/captain/team/${teamid}/fixtures`} className="inline-flex items-center rounded-full border border-white/10 bg-black/20 px-5 py-3 text-sm font-medium text-white/80 transition hover:border-white/20 hover:bg-white/5 hover:text-white">
                Open fixtures
              </Link>
              {canOpenAdminComms ? (
                <Link href={`/admin/teams/${teamid}/communications`} className="inline-flex items-center rounded-full border border-emerald-400/30 bg-emerald-500/15 px-5 py-3 text-sm font-medium text-emerald-50 transition hover:bg-emerald-500/20">
                  Team communications
                </Link>
              ) : null}
            </div>
      ...
      <div className="space-y-6">
        {fixtures.length === 0 ? (
          <section className="rounded-2xl border border-white/10 bg-white/[0.04] p-6 text-sm text-white/60">No upcoming or recently postponed fixtures found.</section>
        ) : null}

        {fixtures.map((fixture) => {
          const availabilityByMemberId = new Map(fixture.availabilities.map((item) => [item.teamMemberId, item]));
          ...
          return (
            <section key={fixture.id} id={`fixture-${fixture.id}`} className="scroll-mt-8 overflow-hidden rounded-3xl border border-white/10 bg-white/[0.04]">
              <div className="border-b border-white/10 px-6 py-5">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                  <div>
                    <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-white/45">Fixture</p>
                    <div className="mt-2 flex flex-wrap items-center gap-2">
                      <h2 className="text-xl font-semibold text-white">{fixture.homeTeam.name} vs {fixture.awayTeam.name}</h2>
                      <span className={`rounded-full border px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.12em] ${fixture.status === "POSTPONED" ? "border-amber-400/25 bg-amber-500/10 text-amber-100" : "border-emerald-400/25 bg-emerald-500/10 text-emerald-100"}`}>
                        {fixture.status.toLowerCase()}
                      </span>
                    </div>
                    <p className="mt-2 text-sm text-white/60">{formatDateTime(fixture.kickoffAt)} - {fixture.venue?.name ?? team.league?.venueName ?? "Venue TBC"}</p>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <Link href={`/captain/team/${teamid}/fixtures/${fixture.id}/selection`} className="inline-flex items-center rounded-full border border-white/10 bg-black/20 px-4 py-1.5 text-xs font-medium text-white/80 transition hover:border-white/20 hover:bg-white/5 hover:text-white">
                      Open selection
                    </Link>
                    <form action={resetFixtureAvailabilityAction}>
                      <input type="hidden" name="teamid" value={teamid} />
                      <input type="hidden" name="fixtureId" value={fixture.id} />
                      <button type="submit" className="inline-flex items-center rounded-full border border-red-400/30 bg-red-500/10 px-4 py-1.5 text-xs font-semibold text-red-100 transition hover:bg-red-500/20" title="Clear old availability replies and SMS chase history for this rearranged fixture.">
                        Reset availability
                      </button>
                    </form>
                  </div>
              ...
              <div className="divide-y divide-white/10">
                {team.members.map((member) => {
                  ...
                  const smsDispatch = smsDispatchBySourceId.get(getSmsSourceId({ fixtureId: fixture.id, teamMemberId: member.id }));
                    ...
                    <div key={member.id} className="grid gap-4 px-6 py-5 xl:grid-cols-[1fr_340px]">
                      <div>
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <span className={`rounded-full border px-2.5 py-1 text-[11px] font-medium ${getResponseClasses(response)}`}>{response.replace("_", " ")}</span>
                        </div>
                        ...
                        <div className="mt-2 space-y-1 text-sm text-white/60">
                          ...
                          <div>
                            {memberPhone ? (
                              <a href={`tel:${memberPhone.replace(/\s+/g, "")}`} className="text-emerald-100/85 underline-offset-4 transition hover:text-emerald-50 hover:underline">
                                {memberPhone}
                              </a>
                            ) : (
                        ...
                        <div className="mt-3 flex flex-wrap gap-2">
                          <div className={`inline-flex rounded-full border px-3 py-1 text-xs font-medium ${getSmsStatusClasses(smsDispatch?.status)}`}>{getSmsStatusText(smsDispatch)}</div>
                          {canOpenAdminComms ? (
                            <Link href={`/admin/teams/${teamid}/players/${member.id}/communications`} className="inline-flex rounded-full border border-emerald-400/30 bg-emerald-500/10 px-3 py-1 text-xs font-medium text-emerald-100 transition hover:bg-emerald-500/15">
                              Player comms
                            </Link>
                          ) : null}
                        </div>

                        {availability?.note ? <div className="mt-2 text-sm text-white/55">Note: {availability.note}</div> : null}
                      </div>
                      ...
                      <div className="space-y-3">
                        <form action={updateFixtureAvailabilityAction} className="space-y-3">
                          ...
                          <FormListboxField name="response" value={response} options={responseOptions} placeholder="Select response" />
                          <input name="note" type="text" defaultValue={availability?.note ?? ""} placeholder="Optional note" className="w-full rounded-xl border border-white/10 bg-black/20 px-3 py-2.5 text-sm text-white outline-none transition focus:border-emerald-500/60" />
                          <button type="submit" className="inline-flex items-center rounded-xl border border-emerald-400/30 bg-emerald-500/10 px-4 py-2.5 text-sm font-medium text-emerald-100 transition hover:bg-emerald-500/15">Save response</button>
                        </form>
                        ...
                        <form action={sendAvailabilitySmsChaseAction}>
                          ...
                          <input type="hidden" name="fixtureId" value={fixture.id} />
                          <input type="hidden" name="teamMemberId" value={member.id} />
                          <button type="submit" disabled={!memberPhone} className="inline-flex w-full items-center justify-center rounded-xl border border-sky-400/30 bg-sky-500/10 px-4 py-2.5 text-sm font-medium text-sky-100 transition hover:bg-sky-500/15 disabled:cursor-not-allowed disabled:border-white/10 disabled:bg-white/[0.03] disabled:text-white/35">
                            {memberPhone ? "Chase by SMS" : "No phone to chase"}
                          </button>
                        </form>

                        <div className="grid gap-2 sm:grid-cols-2">
                          <Link href={`/captain/team/${teamid}/squad/${member.id}/edit`} className="inline-flex w-full items-center justify-center rounded-xl border border-sky-400/25 bg-sky-500/10 px-4 py-2.5 text-center text-sm font-medium text-sky-100 transition hover:bg-sky-500/15">
                            Edit player
                          </Link>
                          {canOpenAdminComms ? (
                            <Link href={`/admin/teams/${teamid}/players/${member.id}/communications`} className="inline-flex w-full items-center justify-center rounded-xl border border-emerald-400/30 bg-emerald-500/10 px-4 py-2.5 text-center text-sm font-medium text-emerald-100 transition hover:bg-emerald-500/15">
                              Player comms
                            </Link>
                          ) : null}
                        </div>
