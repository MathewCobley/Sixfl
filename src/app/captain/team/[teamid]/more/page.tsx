...
export default async function CaptainMorePage({
  ...
  return (
    <main className="mx-auto w-full max-w-xl pb-2">
      <h1 className="px-1 text-lg font-black tracking-tight text-white">More</h1>
      <p className="mt-1 px-1 text-[11px] text-white/38">
        Everything that does not need a permanent bottom tab.
      </p>

      <div className="mt-3 space-y-4">
        {groups.map((group) => (
          <section key={group.label} aria-label={group.label}>
            <h2 className="px-1 text-[10px] font-black uppercase tracking-[0.16em] text-white/35">
              {group.label}
            </h2>
            <div className="mt-1.5 overflow-hidden rounded-[1.05rem] border border-white/[0.07] bg-white/[0.03]">
              {group.rows.map((row, index) => {
                ...
                return (
                  <Link
                    key={row.href}
                    href={row.href}
                    className={`flex min-h-13 items-center gap-3 px-3.5 py-3 active:bg-white/[0.055] ${index > 0 ? "border-t border-white/[0.06]" : ""}`}
                  >
