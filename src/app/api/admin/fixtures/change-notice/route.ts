...
function getStatusNoticeCopy(status: FixtureStatus) {
  if (status === FixtureStatus.CANCELLED) {
    return {
      ...
      intro: "IMPORTANT: this fixture has been cancelled by SIXFL. This cancellation replaces the previous fixture details.",
  ...
  return {
    ...
    intro: "IMPORTANT: this fixture has been postponed by SIXFL. This postponement replaces the previous fixture details.",
...
export async function POST(request: Request) {
  ...
  if (shouldSendStatusNoticeForStatus(status)) {
    ...
    for (const teamId of affectedTeamIds) {
      ...
      const emailBody = [
        ...
        copy.intro,
        "",
        "This status update replaces the previous fixture information.",
        "",
        "What changed:",
      ...
      const smsDispatch = await queueDirectNotification({
        ...
        audience: NotificationAudience.TEAM,
        body: `SIXFL: ${nextHomeTeam.name} vs ${nextAwayTeam.name}, scheduled for ${formatKickoff(nextKickoffAt)}, ${copy.smsAction}. This update replaces the previous fixture details. ${copy.noReconfirm}`,
        isTransactional: true,
        sourceType: STATUS_SOURCE_TYPE,
        sourceId,
        metadata: {
  ...
  for (const teamId of affectedTeamIds) {
    ...
    const smsDispatch = await queueDirectNotification({
      ...
      channel: NotificationChannel.SMS,
      audience: NotificationAudience.TEAM,
      body: `SIXFL fixture update: ${nextHomeTeam.name} vs ${nextAwayTeam.name} has changed. It is now ${formatKickoff(nextKickoffAt)}. Please review and confirm the updated fixture: ${dashboardUrl}`,
      isTransactional: true,
      sourceType: RECONFIRM_SOURCE_TYPE,
      sourceId,
      metadata: { fixtureId: fixture.id, teamId, leagueId, leagueLabel, changeLines },
