...
function getStatusNoticeCopy(status: FixtureStatus) {
  if (status === FixtureStatus.CANCELLED) {
    return {
      ...
      intro: "IMPORTANT: this fixture has been cancelled by SIXFL. Any previous fixture email for this match is no longer correct.",
  ...
  return {
    ...
    intro: "IMPORTANT: this fixture has been postponed by SIXFL. Any previous fixture email for this match is no longer correct.",
...
export async function POST(request: Request) {
  ...
  if (shouldSendStatusNoticeForStatus(status)) {
    ...
    for (const teamId of affectedTeamIds) {
      ...
      const emailBody = [
        ...
        copy.intro,
        "",
        "Please ignore any earlier fixture-live or confirmation email for this old version of the fixture.",
        "",
        "What changed:",
      ...
      const smsDispatch = await queueDirectNotification({
        ...
        audience: NotificationAudience.TEAM,
        body: `SIXFL: ${nextHomeTeam.name} vs ${nextAwayTeam.name} on ${formatKickoff(nextKickoffAt)} ${copy.smsAction}. Ignore any earlier fixture email for the old version. ${copy.noReconfirm}`,
        isTransactional: true,
        sourceType: STATUS_SOURCE_TYPE,
        sourceId,
        metadata: {
  ...
  for (const teamId of affectedTeamIds) {
    ...
    const emailBody = [
      `Hi ${snapshot.primaryContact.name ?? snapshot.teamName},`,
      "",
      "IMPORTANT: a SIXFL fixture involving your team has changed.",
      "",
      "Please ignore any earlier fixture-live or confirmation email for the old version of this fixture. The details below replace the previous details.",
      "",
      "What changed:",
      ...changeLines.map((line) => `- ${line}`),
      "",
      "Updated fixture to use now:",
      newFixtureSummary,
      "",
      "Please check your SIXFL dashboard and confirm this updated fixture only.",
      "",
      "{{cta}}",
      "",
    ...
    const emailDispatch = await queueDirectNotification({
      ...
      channel: NotificationChannel.EMAIL,
      audience: NotificationAudience.TEAM,
      subject: `IMPORTANT fixture update: ${nextHomeTeam.name} vs ${nextAwayTeam.name}`,
      body: emailBody,
      isTransactional: true,
      sourceType: RECONFIRM_SOURCE_TYPE,
      sourceId,
      emailCta: { label: "Open updated fixture", url: dashboardUrl },
    ...
    const smsDispatch = await queueDirectNotification({
      ...
      channel: NotificationChannel.SMS,
      audience: NotificationAudience.TEAM,
      body: `SIXFL fixture update: ignore any earlier email for the old version. ${nextHomeTeam.name} vs ${nextAwayTeam.name} is now ${formatKickoff(nextKickoffAt)}. Confirm updated fixture only: ${dashboardUrl}`,
      isTransactional: true,
      sourceType: RECONFIRM_SOURCE_TYPE,
      sourceId,
      metadata: { fixtureId: fixture.id, teamId, leagueId, leagueLabel, changeLines },
