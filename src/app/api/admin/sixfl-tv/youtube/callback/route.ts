...
export async function GET(request: Request) {
  ...
  if (denied) return publicRedirect(`/admin/sixfl-tv?youtubeError=${encodeURIComponent("Google authorisation was cancelled or denied.")}`);
  if (!code || !state) return publicRedirect(`/admin/sixfl-tv?youtubeError=${encodeURIComponent("Google did not return the required YouTube authorisation details.")}`);
  try {
    ...
    return fixtureId
      ...
      : publicRedirect("/admin/sixfl-tv?youtube=connected");
  } catch (error) {
    const message = error instanceof StudioError ? error.message : "YouTube authorisation could not be completed.";
    return publicRedirect(`/admin/sixfl-tv?youtubeError=${encodeURIComponent(message)}`);
  }
}
