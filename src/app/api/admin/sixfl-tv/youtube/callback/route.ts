...
export async function GET(request: Request) {
  ...
  if (denied) return publicRedirect(`/admin/sixfl-tv/footage?youtubeError=${encodeURIComponent("Google authorisation was cancelled or denied.")}`);
  if (!code || !state) return publicRedirect(`/admin/sixfl-tv/footage?youtubeError=${encodeURIComponent("Google did not return the required YouTube authorisation details.")}`);
  try {
    const fixtureId = await completeYoutubeAuthorisation(code, state, actor);
    return publicRedirect(`/admin/sixfl-tv/footage/${encodeURIComponent(fixtureId)}?youtube=connected`);
  } catch (error) {
    const message = error instanceof StudioError ? error.message : "YouTube authorisation could not be completed.";
    return publicRedirect(`/admin/sixfl-tv/footage?youtubeError=${encodeURIComponent(message)}`);
  }
}
