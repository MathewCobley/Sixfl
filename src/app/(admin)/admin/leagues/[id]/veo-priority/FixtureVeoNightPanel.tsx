...
export default async function FixtureVeoNightPanel({leagueId,date}:{leagueId:string;date:string}) {
 ...
 return <section aria-label="SIXFL TV Priority filming plan" className="space-y-5 rounded-2xl border border-fuchsia-400/25 bg-fuchsia-500/5 p-5 sm:p-6">
  ...
  <p className="text-sm leading-6 text-white/70">Each team has one SIXFL TV Priority Score out of 100. Reliability contributes up to 80 points, audience up to 10 and goal-award participation up to 10. Teams must still meet the reliability and core match-card minimums before the score can earn a camera slot.</p>
   ...
   <div className="space-y-3">{preview.fixtures.filter(f=>!f.bookingState).map(f=>{
     ...
     return <div key={f.id} className="space-y-2 rounded-xl border border-white/15 p-4">
       ...
       <div className="grid gap-2 text-sm text-white/65 sm:grid-cols-2">
         <p>{f.homeName}: <strong>{f.homePriorityScore ?? 0}/100 Priority</strong> · reliability {f.homeReliabilityPoints ?? 0}/80 · audience {f.homeAudiencePoints ?? 0}/10 · participation {f.homeParticipationPoints ?? 0}/10 · View index {f.homeViewScore ?? 100}{f.homePriority?' · eligible & confirmed':' · no active priority'}</p>
         <p>{f.awayName}: <strong>{f.awayPriorityScore ?? 0}/100 Priority</strong> · reliability {f.awayReliabilityPoints ?? 0}/80 · audience {f.awayAudiencePoints ?? 0}/10 · participation {f.awayParticipationPoints ?? 0}/10 · View index {f.awayViewScore ?? 100}{f.awayPriority?' · eligible & confirmed':' · no active priority'}</p>
       </div>
     </div>;
   })}</div>
