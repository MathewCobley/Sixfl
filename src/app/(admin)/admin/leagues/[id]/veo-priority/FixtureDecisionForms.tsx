...
export function RecordingOutcomeForm({leagueId,fixtureId,ready,completed}:{leagueId:string;fixtureId:string;ready:boolean;completed:boolean}) {
 ...
 {!ready&&completed&&<details><summary className="cursor-pointer py-2 font-semibold text-fuchsia-100">Recording ready — add video</summary><form action={action} className="mt-3 space-y-3"><input type="hidden" name="outcome" value="READY"/><label className="block text-sm">YouTube / SIXFL TV link<input name="videoUrl" type="url" required placeholder="https://www.youtube.com/watch?v=…" className="mt-2 min-h-11 w-full rounded-xl border border-white/20 bg-black/20 p-3"/></label><input type="hidden" name="note" value="Usable match recording checked by administrator."/><label className="flex items-start gap-2 text-sm leading-6"><input type="checkbox" name="reviewed" required className="mt-1"/>I have checked that the match recording is usable and the video link is correct.</label><button disabled={pending} className={button}>Save recording</button></form></details>}
 <details><summary className="cursor-pointer py-2 text-amber-100">{ready?'Recording unusable':'Cancel filming / recording failed'}</summary><form action={action} className="mt-3 space-y-3"><input type="hidden" name="outcome" value={completed?'FAILED':'CANCELLED'}/><label className="block text-sm">Reason<textarea name="note" required minLength={5} maxLength={1000} rows={2} className="mt-2 w-full rounded-xl border border-white/20 bg-black/20 p-3"/></label><label className="flex items-start gap-2 text-sm leading-6"><input type="checkbox" name="reviewed" required className="mt-1"/>Close this filming booking. This changes filming status only and does not alter the team’s match fee.</label><button disabled={pending} className={button}>Close filming booking</button></form></details></div>;
}
