...
export default async function AdminRefereesPage({
  ...
  const withLeadCount = referees.filter((referee) =>
    Boolean(referee.createdFromLeadId)
  ).length;
  const totalAssignments = referees.reduce(
    (sum, referee) => sum + referee.refereedFixtures.length,
    0
  );
  const activeAssignments = referees.reduce(
    (sum, referee) =>
      sum +
      referee.refereedFixtures.filter((fixture) => fixture.status === "SCHEDULED")
        .length,
    0
  );
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="space-y-2">
          ...
          <p className="max-w-3xl text-sm leading-7 text-white/60 sm:text-base">
            This is the live referee user directory used by fixtures. Convert
            referee leads first, then manage assignments and contact details
            from here.
          </p>
        </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
          ...
          <div className="mt-1 text-sm text-white/55">
            Converted from referee interest
          </div>
        </div>
      ...
      <div className="rounded-3xl border border-white/10 bg-[radial-gradient(circle_at_top,rgba(16,185,129,0.12),transparent_38%),rgba(255,255,255,0.03)] p-4 shadow-[0_24px_90px_rgba(0,0,0,0.35)] sm:p-5">
        <form className="flex flex-col gap-3 md:flex-row" action="/admin/referees">
          ...
          <div className="flex gap-3">
            <button
              type="submit"
              className="inline-flex h-12 items-center justify-center rounded-2xl bg-emerald-500 px-5 text-sm font-semibold text-black transition hover:bg-emerald-400"
            >
              Search
            </button>
        ...
        {referees.length === 0 ? (
          <div className="rounded-3xl border border-dashed border-white/10 bg-black/20 p-8 text-center">
            ...
            <p className="mt-2 text-sm leading-6 text-white/60">
              Convert a referee lead first, or adjust your search.
            </p>
          ...
          referees.map((referee) => {
            const sourceLead = referee.createdFromLeadId
              ? leadMap.get(referee.createdFromLeadId) ?? null
              : null;

            const nextFixture =
              referee.refereedFixtures.find(
                (fixture) => fixture.status === "SCHEDULED"
              ) ?? referee.refereedFixtures[0] ?? null;

            return (
              <div
                key={referee.id}
                className="overflow-hidden rounded-3xl border border-white/10 bg-black/25 shadow-[0_18px_70px_rgba(0,0,0,0.28)]"
              >
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-sm text-white/60">
                          Earliest linked fixture for this referee.
                        </div>
                      </div>
                    </div>
                    ...
                    {nextFixture ? (
                      <div className="mt-4 rounded-2xl border border-white/10 bg-black/20 p-4">
                        ...
                        <div className="mt-3 text-lg font-bold text-white">
                          {nextFixture.homeTeam?.name || "Home"} v{" "}
                          {nextFixture.awayTeam?.name || "Away"}
                        </div>

                        <div className="mt-2 text-sm text-white/60">
                          {formatDate(nextFixture.kickoffAt)}
                        </div>
                      ...
                      <div className="mt-4 rounded-2xl border border-dashed border-white/10 bg-black/20 p-4">
                        ...
                        <p className="mt-2 text-sm leading-6 text-white/60">
                          This referee is live in the system but has not yet been attached
                          to any fixtures.
                        </p>
                      </div>
                    )}
