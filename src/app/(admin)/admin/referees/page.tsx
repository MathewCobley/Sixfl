...
function buildMailtoHref(input: { email?: string | null; name?: string | null }) {
  const email = input.email?.trim();
  if (!email) return null;

  const subject = encodeURIComponent("SIXFL referee fixtures");
  const greeting = input.name?.trim() ? `Hi ${input.name.trim()},` : "Hi,";
  const body = encodeURIComponent(
    `${greeting}\n\nI am getting in touch about upcoming SIXFL referee fixtures.\n\nThanks,\nSIXFL`,
  );
...
export default async function AdminRefereesPage({
  ...
  const activeAssignments = referees.reduce(
    (sum, referee) =>
      sum + referee.refereedFixtures.filter((fixture) => fixture.status === "SCHEDULED").length,
    0,
  );
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="space-y-2">
          ...
          <p className="max-w-3xl text-sm leading-7 text-white/60 sm:text-base">
            This is the live referee user directory used by fixtures. Contact referees, view their next assignment and manage their fixture links from here.
          </p>
        </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {[
          ["Total referees", totalReferees, "Live assignable referee users"],
          ["From leads", withLeadCount, "Converted from referee interest"],
          ["Total assignments", totalAssignments, "Fixtures linked to referees"],
          ["Scheduled now", activeAssignments, "Upcoming scheduled appointments"],
      ...
      <div className="space-y-4">
        {referees.length === 0 ? (
          ...
          referees.map((referee) => {
            ...
            const mailtoHref = buildMailtoHref({ email: contactEmail, name: referee.name || sourceLead?.contactName });
                  ...
                  <div className="p-5 sm:p-6">
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                      <div className="flex min-w-0 items-start gap-4">
                        <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border border-emerald-500/20 bg-emerald-500/10 text-sm font-black text-emerald-300">
                      ...
                      <div className="flex flex-wrap gap-2">
                        <ContactButton href={mailtoHref} label="Email" primary />
                  ...
                  <div className="border-t border-white/10 bg-white/[0.02] p-5 xl:border-l xl:border-t-0">
                    ...
                    {nextFixture ? (
                      <div className="mt-4 rounded-2xl border border-white/10 bg-black/20 p-4">
                        <div className="flex flex-wrap items-center gap-2">
                          <span className="rounded-full border border-white/10 bg-white/5 px-2.5 py-1 text-[11px] font-bold uppercase tracking-[0.14em] text-white/70">{nextFixture.status}</span>
                          {nextFixture.league ? <span className="rounded-full border border-white/10 bg-white/5 px-2.5 py-1 text-[11px] font-bold uppercase tracking-[0.14em] text-white/70">{nextFixture.league.name}{nextFixture.league.season ? ` • ${nextFixture.league.season}` : ""}</span> : null}
                        </div>
                      ...
                      <div className="mt-4 rounded-2xl border border-dashed border-white/10 bg-black/20 p-4">
                        <div className="text-sm font-semibold text-white">No fixtures assigned yet</div>
                        <p className="mt-2 text-sm leading-6 text-white/60">This referee is live in the system but has not yet been attached to any fixtures.</p>
                      </div>
                    )}
                  </div>
                </div>
