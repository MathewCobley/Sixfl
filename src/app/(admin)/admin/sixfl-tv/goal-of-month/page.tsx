...
export default async function MonthlyGoalAdmin({ searchParams }: { searchParams?: Promise<{ month?: string; saved?: string }> }) {
  ...
  const [rows, page] = await Promise.all([
    prisma.$queryRaw<Row[]>(Prisma.sql`
      SELECT c."id", c."monthKey", c."goalNumber", c."clipAssetId", clip."clipNumber"::int AS "clipNumber",
        c."scorerName", c."status", t."name" AS "teamName",
      ...
      WHERE c."monthKey"=${key} GROUP BY c."id", clip."clipNumber", t."name", f."homeTeamId", away."name", home."name", f."sixflTvUrl"
  ...
  return <div className="space-y-6">
    ...
    <div className="space-y-4">{rows.map(row => <article key={row.id} className="rounded-2xl border border-white/10 p-5"><h3 className="font-bold">{row.teamName} v {row.opponentName} · {row.clipNumber ? `Clip ${row.clipNumber}` : `Goal ${row.goalNumber}`}</h3><p className="my-2 text-sm text-white/60">{row.nominationCount} nominations · {row.voteCount} votes · {row.status}</p><div className="mb-3 flex flex-wrap gap-3">{row.clipAssetId ? <a href={`/api/goal-of-month/clips/${row.id}`} target="_blank" rel="noopener noreferrer" className="text-sm text-emerald-100 underline">Watch exact clip ↗</a> : null}{safeVideoLinks(row.sixflTvUrl).map((url,index) => <a key={url} href={url} target="_blank" rel="noopener noreferrer" className="text-sm text-emerald-100 underline">Watch legacy video {index+1}</a>)}</div><form action={reviewNominee} className="flex flex-wrap items-end gap-3"><input type="hidden" name="candidateId" value={row.id} /><input type="hidden" name="monthKey" value={key} /><label className="text-sm">Scorer <input name="scorerName" defaultValue={row.scorerName ?? ""} maxLength={100} className="block rounded-lg border border-white/20 bg-black p-2" /></label><label className="text-sm">Status <select name="status" defaultValue={row.status} className="block rounded-lg border border-white/20 bg-black p-2"><option value="ACTIVE">Active nominee</option><option value="REMOVED">Removed — keep history</option></select></label><button type="submit" className="rounded-lg bg-emerald-400 px-4 py-2 font-bold text-black">Save changes</button></form></article>)}</div>
