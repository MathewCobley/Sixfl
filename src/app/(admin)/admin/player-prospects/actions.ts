...
function buildYesNoUrls(input: { teamId: string; prospectId: string }) {
...
async function loadProspectForYesNoEmail(input: { prospectId: string; responseTeamId: string | null }) {
  ...
  if (!prospect) return { ok: false as const, error: "Prospect not found." };
  if (prospect.status === "DECLINED" || prospect.status === "DUPLICATE") {
    return { ok: false as const, error: "This prospect is closed and cannot be chased." };
  }
  if (!prospect.email?.trim()) {
    return { ok: false as const, error: "This prospect needs an email address first." };
  }
  ...
  if (!teamId) {
    return { ok: false as const, error: "Choose a team context before sending a YES/NO chase." };
  }

  const selectedTeam = prospect.teamId && prospect.team
    ? prospect.team
  ...
  if (!selectedTeam) return { ok: false as const, error: "Selected team was not found." };

  return { ok: true as const, prospect, team: selectedTeam };
}
...
export async function sendPlayerProspectYesNoChaseAction(formData: FormData) {
  ...
  if (!loaded.ok) {
    redirect(buildRedirectWithParams({ error: loaded.error, leagueId }));
  }
  ...
  if (!email) {
    redirect(buildRedirectWithParams({ error: "This prospect needs an email address first.", leagueId }));
  }

  const leagueName = loaded.team.league
    ? `${loaded.team.league.name}${loaded.team.league.season ? ` — ${loaded.team.league.season}` : ""}`
    : "";
  const urls = buildYesNoUrls({ teamId: loaded.team.id, prospectId: loaded.prospect.id });
  ...
  const dispatch = await queueDirectNotification({
    ...
    subject: `Are you still looking to play for ${loaded.team.name}?`,
    body: [
      "Hi {{firstName}},",
      "",
      "Just checking whether you are still looking to play with {{teamName}} through SIXFL.",
      "",
      "Please tap one of these so we can keep the squad list accurate:",
      "",
      "YES — {{yesResponseUrl}}",
      "NO — {{noResponseUrl}}",
      "",
    ...
    emailBranding: {
      teamName: loaded.team.name,
      teamLogoUrl: loaded.team.logoUrl,
      leagueName,
    },
    metadata: {
      origin: "player_prospect_yes_no_chase",
      originLabel: "Resent YES/NO chase from prospect pool",
      teamId: loaded.team.id,
      prospectId: loaded.prospect.id,
      recipientType: "prospect",
      yesResponseUrl: urls.yesResponseUrl,
      noResponseUrl: urls.noResponseUrl,
      templateKey: "player-prospect-yes-no-chase",
    },
  ...
  await prisma.teamPlayerProspect.update({
    where: { id: loaded.prospect.id },
    data: {
      lastContactedAt: new Date(),
      status: loaded.prospect.status === "NEW" ? "CONTACTED" : undefined,
    },
  });

  await revalidateProspectSurfaces({ prospectId: loaded.prospect.id, teamId: loaded.prospect.teamId ?? loaded.team.id });
