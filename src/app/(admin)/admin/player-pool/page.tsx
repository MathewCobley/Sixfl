...
import { getPlayerPoolContactHistory, playerPoolContactBlock } from "@/lib/player-pool/contact-history";
// ========================================
...
export default async function AdminPlayerPoolPage({
  ...
  const contactHistory = await getPlayerPoolContactHistory(visibleProfiles.map((profile) => profile.id));
          ...
          {visibleProfiles.map((profile) => {
            ...
            return (
              <article key={profile.id} className="rounded-3xl border border-white/10 bg-black/25 p-5">
                ...
                {contactHistory.get(profile.id) ? (
                  <PlayerPoolContactHistory history={contactHistory.get(profile.id)!} prospectId={profile.prospectId} />
                ) : null}
                ...
                <div className="mt-4 flex flex-col gap-3 border-t border-white/10 pt-4 sm:flex-row sm:items-end sm:justify-between">
                  <PlayerPoolNudgeButton
                    profileId={profile.id}
                    playerName={playerName}
                    canNudge={profile.status === "INVITED" && !profile.profileSubmittedAt && Boolean(contactHistory.get(profile.id)) && !playerPoolContactBlock(contactHistory.get(profile.id)!, "EMAIL")}
