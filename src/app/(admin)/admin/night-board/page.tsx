...
function buildNightBoardOverrideScopeKey(input: {
  boardDate: string;
  leagueId: string;
  venueId: string;
}) {
  return `${input.boardDate}::${input.leagueId || "all-leagues"}::${
    input.venueId || "all-venues"
  }`;
}
...
function getTeamChargeSummary(
  ...
  const charges = fixture.paymentCharges.filter(
    (charge) =>
      charge.teamId === teamId && charge.status !== PaymentChargeStatus.VOID,
  );
  ...
  const amountPence = charges.reduce(
    (total, charge) => total + charge.amountPence,
    0,
  );
  ...
  const isPaid =
    outstandingPence === 0 ||
    charges.every((charge) => charge.status === PaymentChargeStatus.PAID);
...
function getTeamConfirmationSummary(
  ...
  if (confirmation.status === "CONFIRMED") {
    return {
      ...
      detail: `${actor} · ${formatDateTime(
        confirmation.confirmedAt ?? confirmation.updatedAt,
      )}`,
  ...
  if (confirmation.status === "ISSUE_RAISED") {
    return {
      ...
      detail: `Raised ${formatDateTime(
        confirmation.issueRaisedAt ?? confirmation.updatedAt,
      )}`,
      note: confirmation.note,
      tone: "issue",
    };
  ...
  return {
    label: "Pending",
    detail: confirmation.lastChasedAt
      ? `Last chased ${formatDateTime(confirmation.lastChasedAt)}`
      : `Created ${formatDateTime(confirmation.createdAt)}`,
    note: confirmation.note,
    tone: "pending",
  };
}

function TeamChargeBadge({
  label,
  teamName,
  summary,
}: {
...
function TeamConfirmationBadge({
  label,
  teamName,
  summary,
}: {
  label: string;
  teamName: string;
  summary: TeamConfirmationSummary;
}) {
  return (
    <div
      className={`rounded-xl border px-3 py-2 text-xs ${confirmationClass(
        summary.tone,
      )}`}
    >
      <div className="flex items-center justify-between gap-3">
...
async function getUpcomingLeagueOptions(): Promise<NightBoardLeagueOption[]> {
...
async function getUpcomingFixtureNightOptions({
  ...
  const grouped = new Map<
    string,
    { date: Date; count: number; leagues: Set<string>; venues: Set<string> }
  >();
  for (const fixture of fixtures) {
  ...
  return Array.from(grouped.entries())
    ...
    .map(([value, group], index) => ({
      ...
      description: `${group.count} fixture${group.count === 1 ? "" : "s"}${
        group.venues.size
          ? ` · ${Array.from(group.venues).slice(0, 2).join(", ")}`
          : ""
      }`,
    }));
}
...
function buildWarnings(fixtures: FixtureForBoard[]) {
  ...
  for (const fixture of fixtures) {
    ...
    if (!pitch) {
      warnings.push({
        level: "amber",
        message: `${formatTime(fixture.kickoffAt)} ${fixture.homeTeam.name} v ${
  ...
  for (const matches of pitchTime.values()) {
    if (matches.length > 1) {
      warnings.push({
        level: "red",
        message: `${formatTime(matches[0].kickoffAt)} has ${matches.length} matches on ${matches[0].pitch}.`,
      });
    }
  }
  for (const matches of refTime.values()) {
    if (matches.length > 1) {
      warnings.push({
        level: "red",
        message: `${
          matches[0].referee?.name ??
          matches[0].referee?.email ??
          "A referee"
        } has ${matches.length} matches at ${formatTime(matches[0].kickoffAt)}.`,
      });
    }
  }
  for (const matches of teamTime.values()) {
    if (matches.length > 1) {
      warnings.push({
        level: "red",
        message: `A team is double-booked at ${formatTime(matches[0].kickoffAt)}.`,
      });
    }
  }
  return warnings;
...
function getBoardGroups(fixtures: FixtureForBoard[]) {
  const pitchNames = Array.from(
    new Set(fixtures.map((fixture) => fixture.pitch?.trim() || "No pitch")),
  ).sort((a, b) => a.localeCompare(b));
  const timeLabels = Array.from(
    new Set(fixtures.map((fixture) => formatTime(fixture.kickoffAt))),
  );
  const fixtureByTimePitch = new Map<string, FixtureForBoard[]>();
  for (const fixture of fixtures) {
    const key = `${formatTime(fixture.kickoffAt)}__${
      fixture.pitch?.trim() || "No pitch"
    }`;
    fixtureByTimePitch.set(key, [
      ...(fixtureByTimePitch.get(key) ?? []),
      fixture,
    ]);
  }
...
async function getRefereeProfileFees(refereeIds: string[]) {
  ...
  try {
    ...
    `);
    return new Map(
      rows.map((row) => [row.userId, row.standardNightFeePence ?? 0]),
    );
  } catch (error) {
...
async function calculateRefereeCosts(
  fixtures: FixtureForBoard[],
  manualRefFeePence: number,
) {
  ...
  const refereeIds = Array.from(
    new Set(
      refedFixtures
        .map((fixture) => fixture.referee?.id)
  ...
  if (manualRefFeePence > 0) {
    ...
    for (const fixture of refedFixtures) {
      ...
      feeByRefereeId.set(
        refereeId,
        (feeByRefereeId.get(refereeId) ?? 0) + manualRefFeePence,
      );
    }
    return {
      totalPence: refedFixtures.length * manualRefFeePence,
      feeByRefereeId,
      sourceLabel: "Manual ref fee per match",
    };
  }
    ...
    try {
      ...
      for (const row of nightRows) {
        ...
        const feePence =
          row.feePence > 0
            ? row.feePence
            : profileFees.get(row.refereeId) ?? 0;
        totalPence += feePence;
        feeByRefereeId.set(
          row.refereeId,
          (feeByRefereeId.get(row.refereeId) ?? 0) + feePence,
        );
      }
    } catch (error) {
  ...
  for (const refereeId of refereeIds) {
    ...
    feeByRefereeId.set(
      refereeId,
      (feeByRefereeId.get(refereeId) ?? 0) + feePence,
    );
  }
  return {
    totalPence,
    feeByRefereeId,
    sourceLabel: "Referee night/profile fees",
  };
}

function getRefereeRows(
  fixtures: FixtureForBoard[],
  feeByRefereeId: Map<string, number>,
) {
  const rows = new Map<
    string,
    {
...
function getFinance(
  ...
  for (const fixture of fixtures) {
    ...
    if (fixture.matchFeePence && fixture.matchFeePence > 0) {
      defaultFeePence = fixture.matchFeePence;
    }
    for (const charge of fixture.paymentCharges) {
      ...
      paidPence += charge.transactions.reduce(
  ...
  const missingChargesPence = Math.max(
    0,
    expectedIncomePence - chargesCreatedPence,
  );
  const outstandingAgainstChargesPence = Math.max(
    0,
    chargesCreatedPence - paidPence,
  );
...
async function calculateAutomaticPitchHire(
  fixtures: FixtureForBoard[],
  nightOverride?: Pick<
    NightBoardOverrideRow,
    "nightPitchCount" | "nightStartTime" | "nightEndTime"
  > | null,
) {
  if (fixtures.length === 0) {
    return { amountPence: 0, label: "No fixtures selected", missingParts: 0 };
  }

  const leagueIds = Array.from(
    new Set(fixtures.map((fixture) => fixture.league.id)),
  );
  const fixtureVenueIds = fixtures
    .map((fixture) => fixture.venue?.id ?? fixture.venueId)
    .filter((id): id is string => Boolean(id));
  const venueNames = fixtures
    .map((fixture) => fixture.venue?.name ?? fixture.league.venueName)
  ...
  const [leagueRows, venueRows] = await Promise.all([
    ...
    venueIds.length > 0 || venueNames.length > 0
      ? prisma.$queryRaw<VenueCostRow[]>(Prisma.sql`
          ...
          WHERE ${
            venueIds.length > 0
              ? Prisma.sql`id IN (${Prisma.join(venueIds)})`
              : Prisma.sql`FALSE`
          }
             OR ${
               venueNames.length > 0
                 ? Prisma.sql`name IN (${Prisma.join(
                     Array.from(new Set(venueNames)),
                   )})`
                 : Prisma.sql`FALSE`
             }
        `)
  ...
  const venueCostById = new Map(
    venueRows.map((row) => [row.venueId, row.defaultPitchCostPerHourPence]),
  );
  const venueCostByName = new Map(
    venueRows.map((row) => [
      row.venueName ?? "",
      row.defaultPitchCostPerHourPence,
    ]),
  );
  const firstFixtureByLeague = new Map<string, FixtureForBoard>();
  for (const fixture of fixtures) {
    if (!firstFixtureByLeague.has(fixture.league.id)) {
      firstFixtureByLeague.set(fixture.league.id, fixture);
    }
  }

  function costForFixture(
    fixture: FixtureForBoard,
    booking?: LeagueBookingCostRow | null,
  ) {
    return (
      booking?.pitchCostPerHourOverridePence ??
      (fixture.venue?.id || fixture.venueId
        ? venueCostById.get(fixture.venue?.id ?? fixture.venueId ?? "") ?? null
        : null) ??
      (fixture.venue?.name || fixture.league.venueName
        ? venueCostByName.get(
            fixture.venue?.name ?? fixture.league.venueName ?? "",
          ) ?? null
        : null)
  ...
  const overrideHours = bookingHours(
    nightOverride?.nightStartTime ?? null,
    nightOverride?.nightEndTime ?? null,
  );
  if (
    nightOverride?.nightPitchCount &&
    nightOverride.nightPitchCount > 0 &&
    overrideHours
  ) {
    ...
    if (costPerHourPence) {
      return {
        amountPence: Math.round(
          nightOverride.nightPitchCount * overrideHours * costPerHourPence,
        ),
        label: `Auto from night override + venue rate: ${
          nightOverride.nightPitchCount
        } pitch${nightOverride.nightPitchCount === 1 ? "" : "es"} × ${formatHours(
          overrideHours,
        )}h × ${formatMoney(costPerHourPence)}/hr`,
        missingParts: 0,
      };
    }
  }
  ...
  for (const leagueId of leagueIds) {
    ...
    const hours = bookingHours(
      booking?.bookingStartTime ?? null,
      booking?.bookingEndTime ?? null,
    );
    ...
    calculatedLeagues += 1;
    labels.push(
      `${pitchCount} pitch${pitchCount === 1 ? "" : "es"} × ${formatHours(
        hours,
      )}h × ${formatMoney(costPerHourPence)}/hr`,
    );
  }
  ...
  if (calculatedLeagues > 0) {
    return {
      ...
      label:
        calculatedLeagues === 1
          ? `Auto from league booking + venue rate: ${labels[0]}`
          : `Auto from ${calculatedLeagues} league bookings + venue rates`,
      missingParts,
    };
  }
  ...
  return {
    amountPence: 0,
    label: "Add league booking hours/pitches and venue hourly rate",
    missingParts: Math.max(missingParts, 1),
  };
}
...
function buildDateOptions(options: SelectOption[], selectedDate: string) {
  if (options.length === 0) {
    return [
      {
        value: selectedDate,
        label: formatDate(new Date(`${selectedDate}T00:00:00.000Z`)),
        description: "No upcoming published fixture nights found",
      },
    ];
  }
...
function FixtureCard({
  ...
  return (
    <div
      className={`rounded-2xl border p-4 ${statusClass(fixture.status)}`}
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="rounded-full border border-white/10 px-2 py-1 text-[10px] uppercase tracking-[0.12em] text-white/55">
          {fixture.status}
        </div>
      </div>
      ...
      <div className="mt-3 grid gap-2">
        <TeamConfirmationBadge
          label="Home confirm"
          teamName={fixture.homeTeam.name}
          summary={homeConfirmation}
        />
        <TeamConfirmationBadge
          label="Away confirm"
          teamName={fixture.awayTeam.name}
          summary={awayConfirmation}
        />
      </div>
      ...
      <div className="mt-3 space-y-1 text-xs text-white/55">
        <div>
          Ref:{" "}
          <span className={fixture.referee ? "text-white/80" : "text-red-200"}>
            {fixture.referee?.name || fixture.referee?.email || "Missing"}
          </span>
        </div>
        <div>
        ...
        <div>
          Venue: {fixture.venue?.name || fixture.league.venueName || "Missing"}
        </div>
        {fixture.result ? (
          <div>
            Result: {fixture.result.homeScore} - {fixture.result.awayScore}
          </div>
        ) : null}
      </div>
      ...
      <NightBoardFixtureEditor
        ...
        statusOptions={FIXTURE_STATUS_OPTIONS.map((statusOption) => ({
          value: statusOption,
          label: statusOption,
        }))}
        locked={fixture.status === FixtureStatus.COMPLETED}
      />

      <div className="mt-3 flex flex-wrap gap-3">
        <Link
          href={`/admin/fixtures/${fixture.id}/edit`}
          className="inline-flex text-xs font-semibold text-sky-200 hover:text-sky-100"
        >
          Full fixture edit
        </Link>
        <Link
          href={`/admin/payments?teamId=${fixture.homeTeam.id}`}
          className="inline-flex text-xs font-semibold text-amber-200 hover:text-amber-100"
        >
          Home payments
        </Link>
        <Link
          href={`/admin/payments?teamId=${fixture.awayTeam.id}`}
          className="inline-flex text-xs font-semibold text-amber-200 hover:text-amber-100"
        >
          Away payments
        </Link>
        <Link
          href={`/admin/fixtures?leagueId=${fixture.league.id}`}
...
export default async function NightBoardPage({
  searchParams,
}: NightBoardPageProps) {
  await requireAdmin();
  ...
  const nightPitchTotalCostValue =
    getSearchParam(params.nightPitchTotalCost) || pitchHireValue;
  ...
  const leagueStillAvailable =
    !leagueId || leagues.some((league) => league.id === leagueId);
  const activeLeagueId = leagueStillAvailable ? leagueId : "";
  let activeVenueId = venueId;
  let activeUpcomingNightOptions = await getUpcomingFixtureNightOptions({
    leagueId: activeLeagueId,
    venueId: activeVenueId,
  });
  if (activeVenueId && activeUpcomingNightOptions.length === 0) {
    activeVenueId = "";
    activeUpcomingNightOptions = await getUpcomingFixtureNightOptions({
      leagueId: activeLeagueId,
      venueId: "",
    });
  }
  ...
  const fixtures = await getFixturesForBoard({
    start,
    end,
    leagueId: activeLeagueId,
    venueId: activeVenueId,
  ...
  const automaticPitchHire = await calculateAutomaticPitchHire(
    fixtures,
    savedOverride,
  );

  const savedManualPitchHirePence =
    savedOverride?.pitchHirePence && savedOverride.pitchHirePence > 0
      ? savedOverride.pitchHirePence
      : null;
  const submittedManualPitchHirePence = parsePositivePence(
    nightPitchTotalCostValue,
  );
  const displayedPitchHireValue =
    nightPitchTotalCostValue || formatMoneyInputValue(savedManualPitchHirePence);
  const displayedPitchCount =
    nightPitchCountValue ||
    (savedOverride?.nightPitchCount ? String(savedOverride.nightPitchCount) : "");
  const displayedStartTime =
    nightStartTimeValue || savedOverride?.nightStartTime || "";
  const displayedEndTime =
    nightEndTimeValue || savedOverride?.nightEndTime || "";
  const pitchHirePence =
    submittedManualPitchHirePence ??
    savedManualPitchHirePence ??
    automaticPitchHire.amountPence;
  const pitchHireSourceLabel =
    submittedManualPitchHirePence || savedManualPitchHirePence
      ? "Saved/manual pitch hire total"
      : automaticPitchHire.label;
  ...
  const leagueOptions = [
    {
      value: "",
      label: "All leagues",
      description: "Upcoming published fixtures only",
    },
    ...leagues.map((league) => ({
      value: league.id,
      label: league.name,
      description: `${league.season ?? "No season"} · ${
        league.fixtureCount
      } upcoming fixture${league.fixtureCount === 1 ? "" : "s"}${
        league.nextKickoffAt ? ` · next ${formatDate(league.nextKickoffAt)}` : ""
  ...
  const refereeOptions = [
    { value: "", label: "No referee" },
    ...referees.map((referee) => ({
      value: referee.id,
      label: `${referee.name || referee.email || "Unnamed referee"}${
        referee.role === UserRole.ADMIN ? " · admin" : ""
      }`,
    })),
  ];

  const warnings = buildWarnings(fixtures);
  const { pitchNames, timeLabels, fixtureByTimePitch } =
    getBoardGroups(fixtures);
  ...
  const isSorted =
    fixtures.length > 0 &&
    warnings.filter((warning) => warning.level === "red").length === 0 &&
    missingPitchCount === 0;
  const hasMissingCharges = finance.missingChargeCount > 0;
  const operationFixtures: NightBoardFixtureOperation[] = fixtures.map(
    (fixture) => ({
      id: fixture.id,
      kickoffTime: toLondonTimeInputValue(fixture.kickoffAt),
      status: fixture.status,
    ...
    <NightBoardOperationsProvider fixtures={operationFixtures}>
      <div className="w-full space-y-8 px-4 pb-10 pt-6 sm:px-6 lg:px-8">
        <AdminCard className="rounded-3xl border border-white/10 bg-white/[0.03] p-6 md:p-8">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-3xl text-sm leading-6 text-white/60">
                One page to check who is playing on which pitch, at what time,
                referee cover, match fees, captain confirmations and whether the
                night is sorted.
              </p>
            </div>
            <div
          ...
          <div className="mt-3 rounded-2xl border border-white/10 bg-black/20 px-4 py-3 text-xs leading-5 text-white/45">
            ...
            <span className="font-semibold text-white/70">
              {formatMoney(pitchHirePence)}
            </span>{" "}
            · {pitchHireSourceLabel}.
          </div>
          <div className="mt-2 rounded-2xl border border-white/10 bg-black/20 px-4 py-3 text-xs leading-5 text-white/45">
            Referee cost:{" "}
            <span className="font-semibold text-white/70">
              {formatMoney(refereeCosts.totalPence)}
            </span>{" "}
            · {refereeCosts.sourceLabel}.
          </div>
        </AdminCard>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <MetricCard
            label="Pitches used"
            value={pitchNames.filter((name) => name !== "No pitch").length}
            detail={`${missingPitchCount} missing pitch`}
          />
          <MetricCard
            label="Referees"
            value={refereeRows.length}
            detail={`${missingRefCount} matches missing ref`}
          />
          <MetricCard
            label="Captain confirms"
            value={`${confirmedCaptains}/${expectedCaptainConfirmations}`}
            detail="home + away confirmations"
          />
        </div>

        <NightBoardPotentialIssuesPanel baseWarnings={warnings} />
        ...
        <AdminCard className="overflow-hidden rounded-3xl border border-white/10 bg-white/[0.03] p-0">
          ...
          {fixtures.length === 0 ? (
            <div className="p-6 text-sm text-white/55">
              No published scheduled/completed fixtures found for these filters.
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="min-w-[1320px] text-left text-sm">
                <thead className="bg-white/[0.03] text-[10px] uppercase tracking-[0.16em] text-white/40">
                  <tr>
                    ...
                    {pitchNames.map((pitch) => (
                      <th key={pitch} className="px-4 py-3">
                        {pitch}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody className="divide-y divide-white/10">
                  {timeLabels.map((time) => (
                    <tr key={time}>
                      <td className="px-4 py-4 align-top text-lg font-semibold text-white">
                        {time}
                      </td>
                      {pitchNames.map((pitch) => {
                        const matches =
                          fixtureByTimePitch.get(`${time}__${pitch}`) ?? [];
                        return (
                          <td
                            key={`${time}-${pitch}`}
                            className="min-w-[410px] px-4 py-4 align-top"
                          >
                            {matches.length === 0 ? (
                              <div className="rounded-2xl border border-white/5 bg-black/20 px-4 py-5 text-center text-white/25">
                                Empty
                              </div>
                            ) : null}
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="text-right text-sm text-white/70">
                      {formatMoney(row.feePence)}
                    </div>
                  </div>
                  <div className="mt-3 grid gap-2 text-sm text-white/55 sm:grid-cols-3">
                    <div>
                      {row.fixtures.length} match
                      {row.fixtures.length === 1 ? "" : "es"}
                    </div>
                    <div>{row.pitchList}</div>
                    <div>
                      {row.firstKickoff ? formatTime(row.firstKickoff) : "—"} –{" "}
                      {row.lastKickoff ? formatTime(row.lastKickoff) : "—"}
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-white/45">
                  Based on published scheduled/completed fixtures on this night.
                </p>
              </div>
              {hasMissingCharges ? (
            ...
            <div className="mt-4 space-y-3 text-sm">
              <div className="flex justify-between border-b border-white/10 pb-2">
                ...
                <span className="font-semibold text-white">
                  {finance.expectedTeams} teams ×{" "}
                  {formatMoney(finance.defaultFeePence)} ={" "}
                  {formatMoney(finance.expectedIncomePence)}
                </span>
              </div>
              <div className="flex justify-between border-b border-white/10 pb-2">
                <span className="text-white/55">Charges created</span>
                <span className="font-semibold text-white">
                  {finance.chargesCreatedCount}/{finance.expectedTeams} teams ={" "}
                  {formatMoney(finance.chargesCreatedPence)}
                </span>
              </div>
              <div className="flex justify-between border-b border-white/10 pb-2">
                <span className="text-white/55">Missing charges</span>
                <span
                  className={`font-semibold ${
                    hasMissingCharges ? "text-amber-100" : "text-emerald-100"
                  }`}
                >
                  {finance.missingChargeCount} teams ={" "}
                  {formatMoney(finance.missingChargesPence)}
                </span>
              </div>
              <div className="flex justify-between border-b border-white/10 pb-2">
                <span className="text-white/55">Paid</span>
                <span className="font-semibold text-emerald-100">
                  {formatMoney(finance.paidPence)}
                </span>
              </div>
              <div className="flex justify-between border-b border-white/10 pb-2">
                <span className="text-white/55">
                  Outstanding against created charges
                </span>
                <span className="font-semibold text-amber-100">
                  {formatMoney(finance.outstandingAgainstChargesPence)}
                </span>
              </div>
              <div className="flex justify-between border-b border-white/10 pb-2">
                <span className="text-white/55">True expected outstanding</span>
                <span className="font-semibold text-amber-100">
                  {formatMoney(finance.expectedOutstandingPence)}
                </span>
              </div>
              <div className="flex justify-between border-b border-white/10 pb-2">
                <span className="text-white/55">Referee cost</span>
                <span className="font-semibold text-white">
                  {formatMoney(finance.refCostPence)}
                </span>
              </div>
              <div className="flex justify-between border-b border-white/10 pb-2">
                <span className="text-white/55">Pitch hire</span>
                <span className="font-semibold text-white">
                  {formatMoney(finance.pitchHirePence)}
                </span>
              </div>
              <div className="flex justify-between pt-2 text-base">
                ...
                <span
                  className={`font-semibold ${
                    finance.expectedProfitPence >= 0
                      ? "text-emerald-100"
                      : "text-red-100"
                  }`}
                >
                  {formatMoney(finance.expectedProfitPence)}
                </span>
              </div>
            </div>
          </AdminCard>
