...
type DivisionTable = DivisionRow & {
  rows: LeagueTableRow[];
};
...
export default async function AdminLeagueTablesPage({ searchParams }: PageProps) {
  ...
  const divisionRows = selectedLeague
  ...
  const divisionTables: DivisionTable[] = selectedLeague
    ? await Promise.all(
        divisionRows.map(async (division) => ({
          ...division,
          rows: await getLeagueTable(selectedLeague.id, { divisionId: division.id }),
        })),
      )
    : [];

  const overallRows = selectedLeague && divisionTables.length === 0
    ? await getLeagueTable(selectedLeague.id)
    : [];

  const tables = divisionTables.length > 0
    ? divisionTables.map((division) => ({
        id: division.id,
        title: division.name,
        rows: division.rows,
      }))
    : selectedLeague
      ? [{ id: selectedLeague.id, title: "League table", rows: overallRows }]
      : [];
  ...
  const completedMatches = Math.floor(
    allRows.reduce((total, row) => total + row.played, 0) / 2,
  );
  const leaders = tables
    .map((table) => table.rows[0]?.teamName)
    .filter((name): name is string => Boolean(name));

  const leagueOptions = leagues.map((league) => ({
    value: league.id,
    label: leagueOptionLabel(league),
  }));
  ...
  return (
    <div className="space-y-7 pb-10">
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-3xl text-sm leading-6 text-white/55 sm:text-base">
              View current standings for any SIXFL league without leaving the admin console.
              Tables use the same completed results and ranking rules as the captain and public pages.
            </p>
          </div>
          ...
          {selectedLeague ? (
            <div className="flex flex-wrap gap-2 lg:justify-end">
              <Link
                href={`/admin/leagues/${selectedLeague.id}`}
                className="inline-flex min-h-11 items-center rounded-2xl border border-sky-400/25 bg-sky-500/10 px-4 py-2 text-sm font-semibold text-sky-100 transition hover:bg-sky-500/15"
              >
                Open league admin
              </Link>
              {selectedLeague.slug ? (
                <Link
                  href={`/leagues/${selectedLeague.slug}`}
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex min-h-11 items-center rounded-2xl border border-white/10 bg-white/[0.05] px-4 py-2 text-sm font-semibold text-white/75 transition hover:bg-white/[0.09] hover:text-white"
                >
                  Open public page
                </Link>
              ) : null}
      ...
      {!selectedLeague ? (
        <section className="rounded-3xl border border-dashed border-white/10 bg-white/[0.025] p-8 text-sm text-white/55">
        ...
        <>
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-white/45">
                  ...
                  {divisionTables.length > 0
                    ? ` · ${divisionTables.length} division${divisionTables.length === 1 ? "" : "s"}`
                    : " · Single table"}
                </p>
              </div>
              <div className="rounded-full border border-emerald-400/20 bg-emerald-500/10 px-3 py-1.5 text-xs font-semibold text-emerald-100">
          ...
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <MetricCard
              label="Teams"
              value={teamIds.size}
              detail="Active teams shown"
            />
            <MetricCard
              label="Completed"
              value={completedMatches}
              detail="Matches in the table"
            />
            <MetricCard
              label="Tables"
              value={tables.length}
              detail={tables.length === 1 ? "One competition table" : "Active divisions"}
            />
            <MetricCard
              label={leaders.length === 1 ? "Leader" : "Leaders"}
              value={leaders.length === 1 ? leaders[0] : leaders.length}
              detail={leaders.length === 0 ? "No results entered yet" : leaders.length === 1 ? "Currently top" : "One per division"}
            />
          </div>
          ...
          <div className="space-y-7">
            {tables.map((table) => (
              <LeagueTableCard
                key={table.id}
                rows={table.rows}
                eyebrow={divisionTables.length > 0 ? selectedLeague.name : "Standings"}
                title={table.title}
                description={
                  divisionTables.length > 0
                    ? `${selectedLeague.season ?? "Current season"} · ${table.rows.length} teams`
                    : `${selectedLeague.name}${selectedLeague.season ? ` · ${selectedLeague.season}` : ""}`
                }
                showTeamLinks={false}
