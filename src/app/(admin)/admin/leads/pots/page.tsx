...
const POT_DEFINITIONS: PotDefinition[] = [
  {
    key: "NEW_INTEREST",
    title: "New player interest",
    description: "Fresh player enquiries that need sorting into the right next step.",
  ...
  },
  {
    ...
    description: "Phone number captured, but no email yet. Ask for email before treating as ready.",
  ...
  },
  {
    ...
    description: "The first request has gone out and the lead is waiting to respond.",
  ...
  },
  {
    ...
    description: "Has an email or mobile, but has not confirmed they definitely want to play.",
  ...
  },
  {
    ...
    description: "They have positively confirmed they want to be considered for a squad.",
  ...
  },
  {
    ...
    description: "Asked for position, level and availability. These help placement but should not block progress.",
  ...
  },
  {
    ...
    description: "Enough information and confirmation to add them into a managed squad.",
  ...
  },
  {
    ...
    description: "Converted player lead. Keep for reporting and conversion tracking.",
  ...
  },
  {
    key: "DORMANT",
    title: "Dormant",
    description: "The chase sequence has finished or the lead has gone quiet. Keep them for future campaigns.",
  ...
  },
  {
    key: "NOT_NOW",
    title: "Not now",
    description: "Interested later or not suitable for the current league build.",
...
function toneClasses(tone: PotDefinition["tone"]) {
  if (tone === "emerald") {
    return {
...
function buildLeadFilterHref(params: { leagueId?: string; pot?: LeadPotKey }) {
  const search = new URLSearchParams();
  search.set("type", "PLAYER");

  if (params.leagueId) search.set("leagueId", params.leagueId);
  if (params.pot) search.set("pot", params.pot);
...
export default async function AdminLeadPotsPage({
  ...
  const summaryByPot = new Map<LeadPotKey, PotSummaryRow>();
      ...
      <div className="space-y-6">
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <div className="mt-1 text-sm text-white/60">
                Same pots, filtered by league. This keeps the system tidy while
                still showing each league build clearly.
              </div>
            </div>
          </div>
        ...
        <div className="grid gap-4 xl:grid-cols-2">
          {POT_DEFINITIONS.map((pot) => {
            ...
            const classes = toneClasses(pot.tone);
              ...
              <table className="min-w-full text-left text-sm">
                ...
                <tbody>
                  {recentLeads.map((lead) => {
                    const definition = POT_DEFINITIONS.find(
                      (pot) => pot.key === lead.leadPot,
                    );
                    const classes = toneClasses(definition?.tone ?? "slate");
