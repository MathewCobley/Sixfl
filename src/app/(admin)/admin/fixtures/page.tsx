...
function buildPublishNotice(input: {
  ...
  const leagueName = input.leagues.find((league) => league.id === leagueId)?.name ?? "this current season";
...
function buildChaseNotice(searchParams: Record<string, string | string[] | undefined>): ChaseNotice | null {
  ...
  if (notice === "sms_queued") return { tone: "success", message: `Chase SMS queued for ${teamName}.` };
  if (notice === "sms_skipped") return { tone: "info", message: `SMS could not be queued for ${teamName}. Check that the team has a usable mobile number and SMS is enabled.` };
  if (notice === "sms_not_available") return { tone: "info", message: `A chase SMS is not available for ${teamName} on this fixture.` };
  if (notice === "sms_error") return { tone: "error", message: "Something went wrong while trying to queue the chase SMS." };
  return null;
}
...
export default async function AdminFixturesPage({ searchParams }: AdminFixturesPageProps) {
  ...
  const publishSummary = leagues.filter((league) => league.id === activeLeagueId).map((league) => {
    ...
    const draftRounds = Array.from(new Set(draftFixtures.map((fixture) => fixture.round).filter((round): round is number => typeof round === "number"))).sort((a, b) => a - b);
    ...
    return { league, total: leagueFixtures.length, drafts, published, scheduled, draftRounds, isActive: true };
  });

  const publishNotice = buildPublishNotice({ searchParams: resolvedSearchParams, leagues: leagues.map((league) => ({ id: league.id, name: league.name })) });
      ...
      <AdminCard className="overflow-hidden rounded-3xl border border-white/10 bg-white/[0.03] p-0 shadow-[0_24px_80px_rgba(0,0,0,0.32)]">
        <div className="border-b border-white/10 px-6 py-6 md:px-8">
          <div className="space-y-2">
            ...
            <p className="max-w-3xl text-sm leading-6 text-white/60">Draft fixtures stay private until you publish them. Only current competition seasons are shown here. Archive seasons stay viewable but cannot be accidentally published from this page.</p>
          </div>
        </div>
