...
export default function CupInvitationComposer({cupId,teams,previewAction,sendAction}:{cupId:string;teams:Choice[];previewAction:CupAction;sendAction:CupAction}) {
  ...
  return <section className="space-y-5 rounded-2xl border border-emerald-400/20 bg-emerald-500/[0.04] p-5">
    ...
    <form action={prepare} className="space-y-4">
      ...
      <button disabled={preparing||!chosen.length} className="rounded-xl bg-emerald-400 px-4 py-3 font-semibold text-black disabled:opacity-40">{preparing?"Preparing…":"Preview selected emails"}</button>
    </form>
    {preview.preview?<div className="space-y-4 border-t border-white/15 pt-5"><h3 className="font-semibold">Review {preview.kind==="REMINDER"?"reminders":"invitations"} for {preview.preview.teams.length} teams</h3>
      <p className="text-sm text-white/60">These preview buttons are not live response links. Each recipient receives a private team-specific confirmation link when you send.</p>
      ...
      <form action={send} className="space-y-3"><input type="hidden" name="cupId" value={cupId}/><input type="hidden" name="kind" value={preview.kind}/><input type="hidden" name="teamIds" value={JSON.stringify(preview.teamIds)}/><input type="hidden" name="previewKey" value={preview.preview.previewKey}/>
