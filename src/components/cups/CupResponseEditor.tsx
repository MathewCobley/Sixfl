...
export default function CupResponseEditor({ cupId, teamId, response, action }: Props) {
  const [state, formAction, pending] = useActionState(action, initialState);

  return (
    <form action={formAction} className="mt-3 max-w-sm space-y-3 rounded-xl border border-white/10 bg-black/20 p-3">
      ...
      {state.error ? <p className="text-xs leading-5 text-amber-200">{state.error}</p> : null}
      {state.success ? <p className="text-xs leading-5 text-emerald-200">{state.success}</p> : null}
    </form>
  );
}
