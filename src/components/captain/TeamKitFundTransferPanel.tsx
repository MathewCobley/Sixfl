  ...
  return (
    <div className="space-y-3">
      <button
        type="button"
        aria-expanded={open}
        aria-controls="captain-kit-fund-panel"
        onClick={() => setOpen((current) => !current)}
        className="flex w-full items-center justify-between gap-4 rounded-2xl border border-sky-400/20 bg-sky-500/[0.07] px-5 py-4 text-left transition hover:border-sky-300/30 hover:bg-sky-500/10"
      >
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-sm font-semibold text-white">Kit fund</span>
            <span className="rounded-full border border-sky-300/20 bg-sky-400/10 px-2.5 py-1 text-xs font-semibold text-sky-100">
              {formatMoney(kitFundBalancePence)}
            </span>
            {teamCreditPence > 0 ? (
              <span className="text-xs text-white/45">
                {formatMoney(teamCreditPence)} team credit available to move
              </span>
            ) : null}
          </div>
          <p className="mt-1 text-xs text-white/45">
            Set money aside for kits. Open only when you need it.
          </p>
        </div>
        <span className="shrink-0 rounded-full border border-white/10 bg-black/20 px-3 py-1.5 text-xs font-semibold text-white/65">
          {open ? "Close" : "Open"}
        </span>
      </button>
      ...
      {open ? (
        <section
          id="captain-kit-fund-panel"
          className="rounded-3xl border border-sky-400/20 bg-sky-500/[0.07] p-5 sm:p-6"
        >
            ...
            <div>
              ...
              <div className="mt-4 flex flex-wrap gap-2 text-xs">
                ...
                <span className="rounded-full border border-sky-300/20 bg-sky-400/10 px-3 py-1.5 text-sky-100">
                  ...
                  {remainderPence > 0
                    ? ` + ${formatMoney(remainderPence)} toward another`
                    : ""}
                </span>
              </div>
                ...
                <div className="mt-5 rounded-2xl border border-white/10 bg-black/20 p-4">
                  ...
                  <div className="mt-3 flex flex-col gap-3 sm:flex-row">
                    ...
                    <button
                      type="button"
                      disabled={
                        busy || amountPence <= 0 || amountPence > teamCreditPence
                      }
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {teamCreditPence >= 2000 ? (
                      <button
                        type="button"
                        disabled={busy}
                        onClick={() => void moveCredit(2000)}
                        className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-1.5 text-xs font-semibold text-white/70 hover:bg-white/[0.08]"
                      >
                        Move £20
                      </button>
                    ) : null}
                    ...
                    <button
                      type="button"
                      disabled={busy}
                      onClick={() => void moveCredit(teamCreditPence)}
                      className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-1.5 text-xs font-semibold text-white/70 hover:bg-white/[0.08]"
                    >
                        ...
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            ...
                            <div className="mt-1 text-xs leading-5 text-white/45">
                              {entry.description || "Kit fund movement"}
                            </div>
                            <div className="mt-1 text-[11px] text-white/35">
                              {new Date(entry.createdAtIso).toLocaleString("en-GB")}
                            </div>
                          </div>
                          <div
                            className={
