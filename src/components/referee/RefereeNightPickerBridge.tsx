...
function createNightButton(night: NightLink) {
  ...
  link.innerHTML = `
    <div class="flex flex-wrap items-center gap-2">
      ...
      ${night.isLegacy ? `<span class="rounded-full border border-amber-400/25 bg-amber-500/15 px-2.5 py-1 text-[11px] font-semibold text-amber-100">Historic / old</span>` : ""}
    </div>
    ...
    <div class="mt-3 text-sm font-semibold ${night.isLegacy ? "text-amber-100" : "text-emerald-100"}">${night.isClosed ? "View night" : "Open night sheet"}</div>
  `;
...
function renderNightPicker() {
  ...
  const primaryNights = current.length > 0 ? current : nights.filter((night) => !night.isClosed).slice(0, 6);
  ...
  const primaryList = primaryNights.map((night) => createNightButton(night).outerHTML).join("");
  const legacyList = legacy.slice(0, 8).map((night) => createNightButton(night).outerHTML).join("");
  ...
  section.innerHTML = `
    <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
      <div>
        ...
        <p class="mt-2 max-w-3xl text-sm leading-6 text-white/60">Older imported nights are kept below as historic records. Current and upcoming nights are shown first so you are not forced into old fixtures.</p>
      </div>
      <div class="rounded-full border border-white/10 bg-black/20 px-3 py-1 text-xs font-medium text-white/55">${nights.length} assigned</div>
    </div>

    <div class="mt-5 grid gap-3 ${primaryNights.length > 1 ? "lg:grid-cols-2" : ""}">
      ${primaryList || `<div class="rounded-2xl border border-dashed border-white/10 bg-black/20 p-4 text-sm text-white/55">No current open night sheets. Historic entries are listed below.</div>`}
    </div>

    ${legacy.length > 0 ? `
      <details class="mt-5 overflow-hidden rounded-2xl border border-amber-400/20 bg-amber-500/10">
        <summary class="cursor-pointer list-none px-4 py-3 text-sm font-semibold text-amber-100 [&::-webkit-details-marker]:hidden">Historic imported nights (${legacy.length})</summary>
        <div class="grid gap-3 border-t border-amber-400/15 p-4 lg:grid-cols-2">${legacyList}</div>
      </details>
    ` : ""}

    ${closed.length > 0 ? `
      <details class="mt-3 overflow-hidden rounded-2xl border border-white/10 bg-black/20">
        <summary class="cursor-pointer list-none px-4 py-3 text-sm font-semibold text-white/75 [&::-webkit-details-marker]:hidden">Submitted / settled nights (${closed.length})</summary>
