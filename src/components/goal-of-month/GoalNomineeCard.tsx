...
export default function GoalNomineeCard({ goal, actionLabel, onAction, disabled, winner }: Props) {
  ...
  const identity = goal.clipNumber ? `Clip ${goal.clipNumber}` : `Goal ${goal.goalNumber}`;
  const label = `${goal.teamName} v ${goal.opponentName} — ${identity}`;
  return (
    <article className="min-w-0 overflow-hidden rounded-2xl border border-white/10 bg-black/30" data-monthly-goal={goal.id}>
      ...
      <div className="space-y-2 p-4">
        <p className="text-xs font-semibold text-fuchsia-200">{new Intl.DateTimeFormat("en-GB", { month: "long", year: "numeric", timeZone: "UTC" }).format(new Date(`${goal.monthKey}-01T12:00:00Z`))} · {identity}</p>
        ...
        <p className="text-xs leading-5 text-white/45">{goal.clipUrl ? `This nomination plays the exact saved Clip ${goal.clipNumber}.` : `Look for goal ${goal.goalNumber} in this fixture’s footage.`}</p>
