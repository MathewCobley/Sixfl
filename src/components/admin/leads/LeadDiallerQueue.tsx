  ...
  async function startCall() {
    ...
    try {
      const response = await fetch("/api/admin/leads/dialler", {
        ...
        body: JSON.stringify({ action: "call", leadId: selected.id }),
      });
      ...
      if (!response.ok || !payload.ok) throw new Error(payload.error || "Could not start call.");
      ...
      setFeedback("Calling your handset now. Answer it and SIXFL will connect the lead.");
    } catch (error) {
      setFeedback(error instanceof Error ? error.message : "Could not start call.");
    } finally {
      setCalling(false);
    }
  }
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(280px,0.8fr)_minmax(0,1.6fr)]">
          ...
          {leads.map((lead) => {
            ...
            return (
              <button
                ...
                className={`w-full border-b border-white/10 px-5 py-4 text-left transition ${active ? "bg-emerald-500/10" : "hover:bg-white/[0.04]"}`}
              >
      ...
      <section className="rounded-3xl border border-white/10 bg-white/[0.035] p-5 sm:p-7">
        ...
        <button
          ...
          onClick={startCall}
          disabled={calling}
          className="mt-6 flex min-h-14 w-full items-center justify-center rounded-2xl bg-emerald-500 px-6 text-lg font-black text-black transition hover:bg-emerald-400 disabled:cursor-wait disabled:opacity-60"
        >
          {calling ? "Starting call…" : `Call ${selected.contactName || selected.teamName || "lead"}`}
        </button>
        <p className="mt-2 text-center text-xs text-white/40">SIXFL calls your configured handset first. When you answer, the lead is connected.</p>
        ...
        <div className="mt-7 border-t border-white/10 pt-6">
          ...
          <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-5">
            <button type="button" disabled={saving} onClick={() => saveOutcome("INTERESTED")} className="rounded-xl border border-emerald-400/30 bg-emerald-500/15 px-3 py-3 text-sm font-bold text-emerald-100">Interested</button>
            <button type="button" disabled={saving} onClick={() => saveOutcome("CALLBACK")} className="rounded-xl border border-sky-400/30 bg-sky-500/15 px-3 py-3 text-sm font-bold text-sky-100">Call back</button>
            <button type="button" disabled={saving} onClick={() => saveOutcome("NO_ANSWER")} className="rounded-xl border border-amber-400/30 bg-amber-500/15 px-3 py-3 text-sm font-bold text-amber-100">No answer</button>
            <button type="button" disabled={saving} onClick={() => saveOutcome("NOT_INTERESTED")} className="rounded-xl border border-red-400/30 bg-red-500/15 px-3 py-3 text-sm font-bold text-red-100">Not interested</button>
            <button type="button" disabled={saving} onClick={() => saveOutcome("JOINED")} className="rounded-xl border border-violet-400/30 bg-violet-500/15 px-3 py-3 text-sm font-bold text-violet-100">Joined</button>
          </div>
        </div>
