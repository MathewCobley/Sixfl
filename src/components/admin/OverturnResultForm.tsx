...
export default function OverturnResultForm(p: Props) {
  ...
  return <details className="rounded-2xl border border-amber-300/30 bg-amber-400/[0.06] p-5">
    ...
    <form action={p.action} className="mt-5 space-y-5">
      ...
      <label className="flex items-start gap-3 text-sm leading-6"><input type="checkbox" name="confirmed" value="yes" className="mt-1 h-4 w-4 shrink-0" checked={confirmed} onChange={e => setConfirmed(e.target.checked)} required/><span>I have verified this is the original score of a completed played match, reviewed the evidence and any approved exception, and given the team an opportunity to respond. I confirm the awarded winner above. The original score and decision will be retained.</span></label>
