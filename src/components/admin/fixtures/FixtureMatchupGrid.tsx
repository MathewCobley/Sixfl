  ...
  useEffect(() => {
    ...
    async function loadGrid() {
      ...
      try {
        ...
        const response = await fetch(`/api/admin/fixtures/matchup-grid?${params.toString()}`, {
          cache: "no-store",
        });
        ...
        setData(result);
        if (!selectedLeagueId && result.selectedLeagueId) {
          setSelectedLeagueId(result.selectedLeagueId);
        }
      } catch {
        ...
        <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-white/60">
              Rows show the team. Columns show the opponent. H means that row team has been Team 1 against that opponent; A means they have been Team 2. This helps spot missing reverse fixtures.
            </p>
          </div>
      ...
      <div className="px-6 py-6 md:px-8">
        ...
        {isLoading ? (
          <div className="mt-6 rounded-2xl border border-white/10 bg-black/20 p-6 text-sm text-white/55">
            Loading matchup grid...
          </div>
        ) : null}

        {!isLoading && (!data || data.teams.length === 0) ? (
          <div className="mt-6 rounded-2xl border border-white/10 bg-black/20 p-6 text-sm text-white/55">
            No teams found for this league yet.
          </div>
        ) : null}
            ...
            <table className="min-w-max border-collapse text-left text-xs">
              <thead>
                <tr className="border-b border-white/10 bg-white/[0.04]">
                  <th className="sticky left-0 z-20 min-w-[180px] border-r border-white/10 bg-[#07120f] px-4 py-3 text-[11px] font-semibold uppercase tracking-[0.16em] text-white/45">
                    Team
                  </th>
                  {data.teams.map((team) => (
                    <th
                      key={team.id}
                      className="min-w-[120px] max-w-[150px] border-r border-white/10 px-3 py-3 text-[11px] font-semibold uppercase tracking-[0.12em] text-white/50"
                    >
              ...
              <tbody>
                {data.cells.map((row) => (
                  <tr key={row.teamId} className="border-b border-white/10 last:border-b-0">
                    <th className="sticky left-0 z-10 min-w-[180px] border-r border-white/10 bg-[#07120f] px-4 py-3 text-sm font-semibold text-white">
                      {row.teamName}
                    </th>
                        ...
                        <div className={`min-h-[72px] rounded-2xl border px-3 py-2 ${getCellTone(cell)}`}>
                          ...
                          {!cell.isSelf ? (
                            <>
                              ...
                              {cell.totalCount > 1 ? (
                                <div className="mt-1 text-[11px] opacity-70">{cell.totalCount} fixtures</div>
                              ) : null}
                            </>
                          ) : null}
                        </div>
