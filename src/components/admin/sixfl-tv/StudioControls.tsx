...
export default function StudioControls({ fixtureId, initial }: { fixtureId: string; initial: State }) {
  ...
  async function generate(kind?: Kind) {
    ...
    try {
      await json(endpoint, {
        action: "render",
        autoPublish: kind !== "HIGHLIGHTS_ALT",
        ...(kind ? { kind } : {}),
      });
      await refresh();
      setMessage(kind
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-2xl border border-emerald-400/20 bg-emerald-400/5 p-5">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="text-xl font-bold text-white">Create SIXFL TV videos</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-white/65">Uses the source files already saved for this fixture. One click generates the normal Highlights and Full match together. Shared intro/outro, the real SIXFL TV logo, saved badges, final score, recorded scorers, pre-match form, saved matchday squads and any stored pre-match SIXFL Predictor score are added by the renderer. The Highlights test style stays completely separate and only runs from its own preview card.</p><p className="mt-2 max-w-3xl text-xs leading-5 text-emerald-100/70">When you press Generate or Regenerate here, each normal video is marked for automatic public YouTube publishing once it finishes and the result is confirmed and undisputed. Background renders created automatically after footage upload stay private until you explicitly generate/regenerate or publish them. If that video has already been published, the new render becomes the live replacement and the older YouTube copy is removed only after the replacement is safely published.</p></div>{publicActiveRenders.length ? <button type="button" className={stopButton} disabled={busy} onClick={() => void stopRendering()}>{busy ? "Stopping…" : "Stop highlights + full match"}</button> : <button type="button" className={button} disabled={busy} onClick={() => void generate()}>{busy ? "Queuing…" : hasReadyPublicPreview ? "Regenerate highlights + full match" : "Generate highlights + full match"}</button>}</div>
