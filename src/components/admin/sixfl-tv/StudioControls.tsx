...
function PublishEditor({ fixtureId, kind, render, thumbnail, publish, connected, busy, onRefresh }: { fixtureId: string; kind: Kind; render?: Render; thumbnail?: Thumbnail; publish?: Publish; connected: boolean; busy: boolean; onRefresh: () => Promise<void> }) {
  ...
  return <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
    ...
    {publish?.state === "READY" && publish.youtubeUrl ? <div className="mt-3 rounded-xl border border-emerald-400/20 bg-emerald-400/5 p-3 text-sm text-emerald-100"><p>Uploaded privately to YouTube and saved against this fixture.</p><a href={publish.youtubeUrl} target="_blank" rel="noreferrer" className="mt-2 inline-block underline underline-offset-4">Open on YouTube</a></div> : null}
    ...
    {active ? <p className="mt-3 text-sm text-white/60">{publish?.state === "QUEUED" ? "Waiting for the worker." : "Uploading the approved video and thumbnail to YouTube as private."}</p> : null}
    <div className="mt-4 grid gap-3">
      <label className="text-sm text-white/70">YouTube title <span className="text-white/40">(optional)</span><input value={title} maxLength={100} onChange={e => setTitle(e.target.value)} placeholder="Leave blank for the automatic match title" className="mt-1 block w-full rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-white outline-none focus:border-emerald-400/40" /></label>
      <label className="text-sm text-white/70">Description <span className="text-white/40">(optional)</span><textarea value={description} maxLength={5000} rows={4} onChange={e => setDescription(e.target.value)} placeholder="Leave blank for the automatic league/date description" className="mt-1 block w-full resize-y rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-white outline-none focus:border-emerald-400/40" /></label>
      <p className="text-xs leading-5 text-amber-100/75">Approval uploads the current finished preview with its current saved thumbnail. The initial YouTube privacy is always <strong>Private</strong>; this button does not email players.</p>
      <button type="button" className={button} disabled={!connected || !ready || active || sending || busy} onClick={() => void approve()}>{sending ? "Approving…" : active ? "Upload in progress…" : "Approve & upload privately to YouTube"}</button>
...
export default function StudioControls({ fixtureId, initial }: { fixtureId: string; initial: State }) {
  ...
  async function generate() {
    ...
    try {
      ...
      await refresh(); setMessage("Preview jobs queued. The isolated video worker will process one video at a time; source footage remains unchanged.");
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-2xl border border-white/10 p-4 text-sm leading-6 text-white/60"><strong className="text-white/85">YouTube connection</strong><br/>{state.youtube.connected ? <>Connected{state.youtube.channelTitle ? ` to ${state.youtube.channelTitle}` : ""}. Every video still needs separate approval below.</> : state.youtube.configured ? <>Google credentials are configured, but the SIXFL YouTube channel has not been authorised. <a className="ml-1 font-semibold text-emerald-300 underline underline-offset-4" href={`/api/admin/sixfl-tv/youtube/start?fixtureId=${encodeURIComponent(fixtureId)}`}>Connect SIXFL YouTube</a></> : <>Google OAuth credentials are not configured yet. Preview generation and thumbnail editing still work without Google.</>}</section>
