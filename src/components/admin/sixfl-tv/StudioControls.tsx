...
type State = { renders: Render[]; thumbnails: Thumbnail[]; youtube: { configured: boolean; connected: boolean; channelId: string | null; channelTitle: string | null } };
...
export default function StudioControls({ fixtureId, initial }: { fixtureId: string; initial: State }) {
  ...
  const active = useMemo(() => state.renders.some(render => render.state === "QUEUED" || render.state === "PROCESSING"), [state.renders]);
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-4 lg:grid-cols-2">{(["HIGHLIGHTS", "FULL_MATCH"] as Kind[]).map(kind => {
      ...
      return <section key={kind} className="rounded-2xl border border-white/10 bg-white/[0.03] p-4"><div className="flex flex-wrap items-center justify-between gap-3"><h3 className="font-semibold text-white">{kindLabel(kind)} preview</h3><span className="text-xs text-white/50">{render ? render.state.replace("_", " ") : "Not generated"}</span></div>
    ...
    <section className="rounded-2xl border border-white/10 p-4 text-sm leading-6 text-white/60"><strong className="text-white/85">YouTube connection</strong><br/>{state.youtube.connected ? `Connected${state.youtube.channelTitle ? ` to ${state.youtube.channelTitle}` : ""}. Publishing still requires an explicit approval button.` : state.youtube.configured ? "Google credentials are configured, but the SIXFL YouTube channel has not been authorised yet." : "Google OAuth credentials are not configured yet. Preview generation and thumbnail editing do not require Google."}</section>
  </div>;
}
