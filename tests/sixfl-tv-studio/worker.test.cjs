...
test('actual FFmpeg assembly reconstructs saved manifests and produces a decodable private MP4', { timeout: 90000 }, async t => {
  ...
  assert.ok(meta.streams.some(s => s.codec_type === 'audio')); assert.ok(Number(meta.format.duration) >= 24.0);
  ...
  const predictorFrame = await w.run('ffmpeg', ['-ss', '10.0', '-i', result, '-frames:v', '1', '-f', 'md5', '-'], true);
  const resultFrame = await w.run('ffmpeg', ['-ss', '14.0', '-i', result, '-frames:v', '1', '-f', 'md5', '-'], true);
  const goalFrame = await w.run('ffmpeg', ['-ss', '21.0', '-i', result, '-frames:v', '1', '-f', 'md5', '-'], true);
  assert.equal(new Set([titleFrame,lineupFrame,predictorFrame,resultFrame,goalFrame]).size,5,'Scored match card, matchday squad, AI Predictor, full-time score/scorers and Goal of the Month end card must all survive assembly');
