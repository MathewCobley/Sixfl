...
test('studio queues saved sources in editing order and requires confirmed result', async t=>{
  ...
  try{
    ...
    await applySql(db,'prisma/migrations/20260917170000_sixfl_tv_footage_uploads/migration.sql');await applySql(db,'prisma/migrations/20260917213000_sixfl_tv_studio/migration.sql');await applySql(db,'prisma/migrations/20260918182000_goal_of_month_saved_clips/migration.sql');
    const add=async(id,fixtureId,kind,position,clipNumber=null)=>db.$executeRaw`INSERT INTO "SixflTvFootageAsset" ("id","fixtureId","kind","filename","sizeBytes","lastModified","partCount","position","clipNumber","createdByActor","state","completedAt") VALUES (${id},${fixtureId},${kind},${id+'.mp4'},24,1,1,${position},${clipNumber},'admin','READY',NOW())`;
    await add('intro',null,'INTRO',0);await add('clip-a','match-a','CLIP',0,1);await add('clip-b','match-a','CLIP',1,2);await add('ready-highlights','match-a','HIGHLIGHTS',0);await add('full','match-a','FULL_MATCH',0);await add('outro',null,'OUTRO',0);
