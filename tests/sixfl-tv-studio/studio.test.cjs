...
test('studio queues saved sources in editing order and requires confirmed result', async t=>{
  ...
  try{
    ...
    assert.ok(jobs.every(x=>Number(x.metadataJson.renderVersion)===14),'Renderer version must invalidate old finished previews after editing changes');
