...
test('studio source keeps publishing explicit, public and isolated from customer notifications',()=>{
  ...
  assert.match(youtube,/aes-256-gcm/);assert.match(youtube,/youtube\.upload/);assert.match(youtube,/youtube\.force-ssl/);assert.match(youtube,/replacementCleanupEnabled/);assert.match(youtube,/access_type/);assert.match(youtube,/offline/);assert.match(youtube,/syncPublishedYoutubeThumbnail/);assert.match(youtube,/thumbnails\/set/);
  assert.match(settings,/Reconnect to enable replacement cleanup/);assert.match(settings,/older SIXFL copies can be removed from YouTube/);
