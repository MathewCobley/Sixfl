...
test('studio queues saved sources in editing order and requires confirmed result', async t=>{
  ...
  try{
    ...
    assert.ok(jobs.every(x=>Number(x.metadataJson.renderVersion)===20),'Renderer version must invalidate old finished previews after faithful alternative highlights match-card redesign');
