...
test('studio source keeps publishing explicit, private and isolated from customer notifications',()=>{
  ...
  assert.match(graphics,/brandingAsset\(siteUrl, "\/Sixfl-tv\.png"\)/);assert.match(graphics,/brandingAsset\(siteUrl, "\/logos\/sixfl-ai-predictor\.png"\)/);assert.match(graphics,/sixflFont\(siteUrl, "\/fonts\/Inter-Regular\.ttf"\)/);assert.match(graphics,/sixflFont\(siteUrl, "\/fonts\/Inter-Bold\.ttf"\)/);assert.doesNotMatch(graphics,/node:fs|readFile\(|process\.cwd\(\)/);assert.doesNotMatch(graphics,/>SIXFL TV<\/text>/);assert.doesNotMatch(graphics,/>SIXFL PREDICTOR/);assert.match(graphics,/RECENT FORM/);assert.match(graphics,/PRE-MATCH PREDICTION/);assert.match(graphics,/typeLabel = isHighlights \? "HIGHLIGHTS" : "FULL MATCH"/);assert.match(graphics,/thumbnailFloodlights/);assert.match(graphics,/nightSky/);assert.match(graphics,/pitchGrad/);assert.match(graphics,/scoreShadow/);assert.match(graphics,/scorerLinesForTeam/);assert.match(graphics,/font-size="23" font-weight="900" fill="#34d399">•<\/text>/);assert.match(graphics,/broadcastCodeForTeam/);assert.doesNotMatch(graphics,/GOALSCORERS/);assert.match(graphics,/monthUpper/);assert.match(graphics,/Nominate until 5/);assert.match(graphics,/winner announced from 13/);assert.match(graphics,/width="690" height="94"/);assert.match(graphics,/LEAGUE TABLE/);assert.match(graphics,/TOP HALF/);assert.match(graphics,/BOTTOM HALF/);assert.match(graphics,/row\.movement === "UP"/);assert.match(graphics,/y="344"/);assert.match(graphics,/teamNameY = predictor \? 470 : 330/);assert.match(graphics,/Match highlights/);assert.match(graphics,/Full match/);assert.match(graphics,/list\(first, 250\)/);assert.match(graphics,/list\(second, 1210\)/);
