...
test('studio source keeps publishing explicit, public and isolated from customer notifications',()=>{
  ...
  assert.match(ui,/Publish \/ retry now/);assert.match(ui,/publish automatically as/);assert.match(ui,/Regenerate highlights \+ full match/);assert.match(ui,/Highlights test style stays completely separate/);assert.match(ui,/Regenerate \$\{kindLabel\(kind\)\} only/);assert.match(ui,/action: "render", \.\.\.\(kind \? \{ kind \} : \{\}\)/);assert.match(ui,/publish\?\.title \|\| defaults\.title/);assert.match(ui,/publish\?\.description \|\| defaults\.description/);assert.match(ui,/thumbnailDraft\.headline/);assert.match(ui,/headline: thumbnailDraft\.headline/);assert.match(ui,/strapline: thumbnailDraft\.strapline/);assert.match(ui,/showScore: thumbnailDraft\.showScore/);assert.match(ui,/Live preview/);assert.match(ui,/renderRevision/);assert.match(ui,/preview: "1"/);assert.match(ui,/Stop highlights \+ full match/);assert.match(ui,/cancel-render/);assert.match(thumbRoute,/thumbnailPreviewResponse/);assert.doesNotMatch(ui,/<select\b|MutationObserver|document\.querySelector/);
