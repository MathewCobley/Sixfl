...
test('studio source keeps publishing explicit, public and isolated from customer notifications',()=>{
  ...
  assert.match(ui,/Approve & publish publicly to YouTube/);assert.match(ui,/publish\?\.title \|\| defaults\.title/);assert.match(ui,/publish\?\.description \|\| defaults\.description/);assert.match(ui,/thumbnailDraft\.headline/);assert.match(ui,/headline: thumbnailDraft\.headline/);assert.match(ui,/strapline: thumbnailDraft\.strapline/);assert.match(ui,/showScore: thumbnailDraft\.showScore/);assert.match(ui,/Live preview/);assert.match(ui,/renderRevision/);assert.match(ui,/preview: "1"/);assert.match(ui,/Stop rendering/);assert.match(ui,/cancel-render/);assert.match(thumbRoute,/thumbnailPreviewResponse/);assert.doesNotMatch(ui,/<select\b|MutationObserver|document\.querySelector/);
  assert.match(api,/confirmed !== true/);assert.match(api,/cancel-render/);assert.match(api,/syncPublishedYoutubeThumbnail/);assert.match(api,/await saveThumbnail\(fixtureId, kind, actor, data\)[\s\S]{0,180}queueYoutubePublish\(fixtureId, kind, actor, data\)/,'Publishing must save the exact live thumbnail before queuing YouTube');assert.match(worker,/privacyStatus: "private" \| "unlisted" \| "public"/);assert.match(worker,/privacyStatus: job\.privacyStatus/);assert.match(worker,/assignYoutubeSubscriberNotification/);assert.match(worker,/notificationDay/);assert.match(worker,/notifySubscribers", notifySubscribers \? "true" : "false"/);assert.match(worker,/thumbnails\/set/);assert.match(worker,/buildSixflTvVideoValue/);assert.match(worker,/RENDER_HEARTBEAT_MS = 5000/);
