...
test('studio queues saved sources in editing order and requires confirmed result', async t=>{
  ...
  try{
    ...
    await applySql(db,'prisma/migrations/20260917170000_sixfl_tv_footage_uploads/migration.sql');await applySql(db,'prisma/migrations/20260917213000_sixfl_tv_studio/migration.sql');await db.$executeRawUnsafe('ALTER TABLE "SixflTvFootageAsset" ADD COLUMN "clipNumber" INTEGER');
    ...
    assert.ok(jobs.every(x=>Number(x.metadataJson.renderVersion)===16),'Renderer version must invalidate old finished previews after clip labels');
