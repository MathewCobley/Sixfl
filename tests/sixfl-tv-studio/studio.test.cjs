...
test('fixture graphics generate deterministic YouTube and video-card PNG shapes', async()=>{
  ...
  try{
    ...
    const tm=await sharp(thumb).metadata(), ftm=await sharp(fullThumb).metadata(), cm=await sharp(card).metadata(), gm=await sharp(goal).metadata(), sm=await sharp(scoreBug).metadata(), lm=await sharp(lineup).metadata();
...
test('studio queues saved sources in editing order and requires confirmed result', async t=>{
  ...
  try{
    ...
    assert.ok(jobs.every(x=>Number(x.metadataJson.renderVersion)===10),'Renderer version must invalidate old finished previews after editing changes');
...
test('studio source keeps publishing explicit, private and isolated from customer notifications',()=>{
  ...
  assert.match(worker,/swipeVideo/);assert.match(worker,/LINEUP_SECONDS/);assert.doesNotMatch(worker,/PREDICTOR_SECONDS/);assert.match(worker,/RESULT_SECONDS/);assert.match(worker,/GOAL_OF_MONTH_END_SECONDS/);assert.match(worker,/goalOfMonthBeforeOutro=1/);assert.match(worker,/createSixflTvScoreBug/);assert.match(worker,/createSixflTvLineupCard/);assert.doesNotMatch(worker,/createSixflTvPredictorCard/);assert.match(worker,/predictorOnLineup=\$\{metadata\.fixture\.predictor/);assert.match(worker,/if \(swipe\) segments\.push\(swipe\);\n    for \(let index = 0/,'A swipe must lead into the first content clip');assert.match(worker,/if \(swipe\) segments\.push\(swipe\);[\s\S]{0,180}const result/,'A swipe must lead from the final clip into the result card');assert.ok(worker.indexOf('await cardVideo(goalOfMonthPng') < worker.indexOf('Rendering outro'),'Goal of the Month must run before the outro');assert.match(worker,/footageOverlay=\$\{job\.kind/);
  assert.match(graphics,/brandingAsset\(siteUrl, "\/Sixfl-tv\.png"\)/);assert.match(graphics,/brandingAsset\(siteUrl, "\/logos\/sixfl-ai-predictor\.png"\)/);assert.match(graphics,/sixflFont\(siteUrl, "\/fonts\/Inter-Regular\.ttf"\)/);assert.match(graphics,/sixflFont\(siteUrl, "\/fonts\/Inter-Bold\.ttf"\)/);assert.doesNotMatch(graphics,/node:fs|readFile\(|process\.cwd\(\)/);assert.doesNotMatch(graphics,/>SIXFL TV<\/text>/);assert.doesNotMatch(graphics,/>SIXFL PREDICTOR/);assert.match(graphics,/RECENT FORM/);assert.match(graphics,/PRE-MATCH PREDICTION/);assert.match(graphics,/typeLabel = isHighlights \? "HIGHLIGHTS" : "FULL MATCH"/);assert.match(graphics,/badgeGlow/);assert.match(graphics,/scoreShadow/);assert.match(graphics,/scorerLinesForTeam/);assert.match(graphics,/broadcastCodeForTeam/);assert.doesNotMatch(graphics,/GOALSCORERS/);assert.match(graphics,/monthUpper/);assert.match(graphics,/Nominate until 5/);assert.match(graphics,/winner announced from 13/);assert.match(graphics,/width="820" height="116"/);assert.match(graphics,/teamNameY = predictor \? 470 : 330/);
