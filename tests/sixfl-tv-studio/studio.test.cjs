...
test('studio source keeps publishing explicit, public and isolated from customer notifications',()=>{
  ...
  assert.match(ui,/Publish \/ retry now/);assert.match(ui,/publish automatically as/);assert.match(ui,/Regenerate highlights \+ full match/);assert.match(ui,/Highlights test style stays completely separate/);assert.match(ui,/Regenerate \$\{kindLabel\(kind\)\} only/);assert.match(ui,/action: "render",\s*autoPublish: kind !== "HIGHLIGHTS_ALT",\s*\.\.\.\(kind \? \{ kind \} : \{\}\)/);assert.match(ui,/publish\?\.title \|\| defaults\.title/);assert.match(ui,/publish\?\.description \|\| defaults\.description/);assert.match(ui,/thumbnailDraft\.headline/);assert.match(ui,/headline: thumbnailDraft\.headline/);assert.match(ui,/strapline: thumbnailDraft\.strapline/);assert.match(ui,/showScore: thumbnailDraft\.showScore/);assert.match(ui,/Live preview/);assert.match(ui,/renderRevision/);assert.match(ui,/preview: "1"/);assert.match(ui,/Stop highlights \+ full match/);assert.match(ui,/cancel-render/);assert.match(thumbRoute,/thumbnailPreviewResponse/);assert.doesNotMatch(ui,/<select\b|MutationObserver|document\.querySelector/);
  assert.match(api,/confirmed !== true/);assert.match(api,/cancel-render/);assert.match(youtube,/pg_advisory_xact_lock\(76424424\)/);assert.match(youtube,/alreadyPublished/);assert.match(youtube,/alreadyQueued/);assert.match(api,/requestRenders\(fixtureId, actor, kind, \{\s*autoPublish: data\.autoPublish !== false,/);assert.match(api,/syncPublishedYoutubeThumbnail/);assert.match(worker,/cleanupOlderYoutubeCopies/);assert.match(worker,/cleanupOneSupersededYoutubeVideo/);assert.match(worker,/method: "DELETE"/);assert.match(worker,/Superseded by a newer SIXFL TV publish and removed from YouTube/);assert.match(worker,/fixture does not point at newest published video/);assert.match(api,/await saveThumbnail\(fixtureId, kind, actor, data\)[\s\S]{0,180}queueYoutubePublish\(fixtureId, kind, actor, data\)/,'Publishing must save the exact live thumbnail before queuing YouTube');assert.match(worker,/privacyStatus: "private" \| "unlisted" \| "public"/);assert.match(worker,/privacyStatus: job\.privacyStatus/);assert.match(worker,/queueAutomaticYoutubePublish/);assert.match(worker,/automatic-youtube-publish/);assert.match(worker,/mr\."isDisputed"=FALSE/);assert.match(worker,/createSixflTvThumbnail/);assert.match(worker,/assignYoutubeSubscriberNotification/);assert.match(worker,/notificationDay/);assert.match(worker,/notifySubscribers", notifySubscribers \? "true" : "false"/);assert.match(worker,/thumbnails\/set/);assert.match(worker,/buildSixflTvVideoValue/);assert.match(worker,/RENDER_HEARTBEAT_MS = 5000/);
...
test('new normal renders auto publish while old READY previews stay excluded',()=>{
  ...
  const restoreMigration=fs.readFileSync('prisma/migrations/20260924015500_restore_accidental_youtube_replacements/migration.sql','utf8');
  const cutoverMigration=fs.readFileSync('prisma/migrations/20260924025000_auto_publish_new_tv_renders/migration.sql','utf8');

  assert.match(ui,/autoPublish: kind !== "HIGHLIGHTS_ALT"/);
  assert.match(ui,/Every newly created normal Highlights or Full match render automatically publishes publicly on YouTube/);
  assert.match(ui,/Old READY previews from before this change stay private/);
  assert.match(api,/autoPublish: data\.autoPublish !== false/);
  assert.match(studio,/autoYoutubePublishRequested/);
  assert.match(studio,/options\?\.autoPublish !== false/);

  // The worker still requires an explicit per-job opt-in bit. This is what
  // prevents a restart from sweeping up old historical READY rows.
  ...
  // The one-time cutover opts in only tonight's fresh batch. Older restored
  // videos remain untouched and cannot be republished just by restarting.
  assert.match(cutoverMigration,/2026-09-24 01:25:00\+00/);
  assert.match(cutoverMigration,/"kind" IN \('HIGHLIGHTS','FULL_MATCH'\)/);
  assert.match(cutoverMigration,/"state" IN \('QUEUED','PROCESSING','READY'\)/);
  assert.match(cutoverMigration,/NOT EXISTS[\s\S]*"SixflTvYoutubePublish"/);
  assert.match(cutoverMigration,/autoYoutubePublishRequested/);

  assert.match(restoreMigration,/Dynamo Kebab 3-1 Inter Maignan/);
  assert.match(restoreMigration,/Taking Part FC 4-2 The Units/);
  assert.match(restoreMigration,/Original SIXFL highlights link restored/);
});
