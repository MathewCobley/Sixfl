...
test('studio queues saved sources in editing order and requires confirmed result', async t=>{
  ...
  try{
    ...
    const first=await studio.requestRenders('match-a','admin');assert.equal(first.renders.length,3);
    ...
    assert.ok(jobs.every(x=>Number(x.metadataJson.renderVersion)===22),'Renderer version must invalidate old finished previews after faithful alternative highlights match-card redesign');
...
test('studio source keeps publishing explicit, public and isolated from customer notifications',()=>{
  ...
  assert.match(graphics,/replace\(\/\[\^A-Z0-9&\]\/g, ""\)/);assert.match(graphics,/brandingAsset\(siteUrl, "\/Sixfl-tv\.png"\)/);assert.match(graphics,/brandingAsset\(siteUrl, "\/logos\/sixfl-ai-predictor\.png"\)/);assert.match(graphics,/sixflFont\(siteUrl, "\/fonts\/Inter-Regular\.ttf"\)/);assert.match(graphics,/sixflFont\(siteUrl, "\/fonts\/Inter-Bold\.ttf"\)/);assert.doesNotMatch(graphics,/node:fs|readFile\(|process\.cwd\(\)/);assert.match(graphics,/createSixflTvAltHighlightsOverlay/);const altStart=graphics.indexOf('export async function createSixflTvAltHighlightsOverlay');const altEnd=graphics.indexOf('export async function createSixflTvAltVideoCard',altStart);const altOverlay=graphics.slice(altStart,altEnd);assert.match(altOverlay,/>MATCH HIGHLIGHTS<\/text>/);assert.doesNotMatch(altOverlay,/>NO\.<\/text>/);assert.doesNotMatch(graphics,/>SIXFL PREDICTOR/);assert.match(graphics,/RECENT FORM/);assert.match(graphics,/PRE-MATCH PREDICTION/);assert.match(graphics,/backgroundImage\?: Uint8Array \| null/);assert.match(graphics,/MATCHWEEK /);assert.doesNotMatch(graphics,/MATCHWEEK NO\./);assert.match(graphics,/matchweekNumber/);assert.match(graphics,/footerText, left: 670, top: 650/);assert.match(graphics,/scorebarX = isHighlights \? 34 : 1280 - 34 - 450/);assert.match(graphics,/fallbackBg/);assert.match(graphics,/bottomShade/);assert.match(graphics,/scoreShadow/);assert.match(graphics,/scorerLinesForTeam/);assert.match(graphics,/font-size="23" font-weight="900" fill="#34d399">•<\/text>/);assert.match(graphics,/broadcastCodeForTeam/);assert.doesNotMatch(graphics,/GOALSCORERS/);assert.match(graphics,/monthUpper/);assert.match(graphics,/Nominate until 5/);assert.match(graphics,/winner announced from 13/);assert.match(graphics,/width="690" height="94"/);assert.match(graphics,/LEAGUE TABLE/);assert.match(graphics,/TOP HALF/);assert.match(graphics,/BOTTOM HALF/);assert.match(graphics,/row\.movement === "UP"/);assert.match(graphics,/y="344"/);assert.match(graphics,/teamNameY = predictor \? 470 : 330/);assert.doesNotMatch(graphics,/typeSubtitle/);assert.match(graphics,/list\(first, 250\)/);assert.match(graphics,/list\(second, 1210\)/);
