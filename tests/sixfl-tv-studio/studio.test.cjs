...
test('fixture graphics generate deterministic YouTube and video-card PNG shapes', async()=>{
  ...
  try{
    ...
    const fixture={leagueName:'Northallerton Wednesday · Autumn 2026',kickoffLabel:'Thu, 17 Sep 2026',kickoffIso:'2026-09-17T18:30:00.000Z',firstTeam:{name:'Town Hall 6s',logoUrl:null,broadcastCode:'TH6',score:4},secondTeam:{name:'Ballerz FC',logoUrl:null,broadcastCode:'BFC',score:2},scorers:['Town Hall 6s: Alex One x2, Sam Two','Ballerz FC: Chris Three']};
    ...
    const tm=await sharp(thumb).metadata(), ftm=await sharp(fullThumb).metadata(), cm=await sharp(card).metadata(), gm=await sharp(goal).metadata(), sm=await sharp(scoreBug).metadata(), lm=await sharp(lineup).metadata(), ttm=await sharp(tableTop).metadata(), tbm=await sharp(tableBottom).metadata();
...
test('studio queues saved sources in editing order and requires confirmed result', async t=>{
  ...
  try{
    ...
    assert.ok(jobs.every(x=>Number(x.metadataJson.renderVersion)===11),'Renderer version must invalidate old finished previews after editing changes');
    ...
    assert.equal(graphic.firstTeam.broadcastCode,'TH6');assert.equal(graphic.secondTeam.broadcastCode,'BFC');
...
test('studio source keeps publishing explicit, private and isolated from customer notifications',()=>{
  ...
  assert.match(ui,/Approve & upload privately to YouTube/);assert.match(ui,/Live preview/);assert.match(ui,/preview: "1"/);assert.match(ui,/Stop rendering/);assert.match(ui,/cancel-render/);assert.match(thumbRoute,/thumbnailPreviewResponse/);assert.doesNotMatch(ui,/<select\b|MutationObserver|document\.querySelector/);
  assert.match(api,/confirmed !== true/);assert.match(api,/cancel-render/);assert.match(worker,/privacyStatus: "private"/);assert.match(worker,/notifySubscribers/);assert.match(worker,/thumbnails\/set/);assert.match(worker,/buildSixflTvVideoValue/);assert.match(worker,/RENDER_HEARTBEAT_MS = 5000/);
  assert.match(worker,/swipeVideo/);assert.match(worker,/LINEUP_SECONDS/);assert.doesNotMatch(worker,/PREDICTOR_SECONDS/);assert.doesNotMatch(worker,/RESULT_SECONDS/);assert.match(worker,/GOAL_OF_MONTH_END_SECONDS/);assert.match(worker,/LEAGUE_TABLE_SECONDS/);assert.match(worker,/resultCard=0/);assert.match(worker,/goalOfMonthAfterFootage=1/);assert.match(worker,/createSixflTvScoreBug/);assert.match(worker,/createSixflTvLineupCard/);assert.match(worker,/createSixflTvLeagueTableCard/);assert.doesNotMatch(worker,/createSixflTvPredictorCard/);assert.match(worker,/predictorOnLineup=\$\{metadata\.fixture\.predictor/);assert.match(worker,/if \(swipe\) segments\.push\(swipe\);\n    for \(let index = 0/,'A swipe must lead into the first content clip');assert.match(worker,/if \(swipe\) segments\.push\(swipe\);[\s\S]{0,220}Adding Goal of the Month card/,'The final clip must transition directly into Goal of the Month');assert.ok(worker.indexOf('await cardVideo(goalOfMonthPng') < worker.indexOf('Adding top-half league table'),'Goal of the Month must precede the league table');assert.ok(worker.indexOf('Adding top-half league table') < worker.indexOf('Rendering outro'),'League table must be shown before the outro');assert.match(worker,/footageOverlay=\$\{job\.kind/);
  assert.match(graphics,/brandingAsset\(siteUrl, "\/Sixfl-tv\.png"\)/);assert.match(graphics,/brandingAsset\(siteUrl, "\/logos\/sixfl-ai-predictor\.png"\)/);assert.match(graphics,/sixflFont\(siteUrl, "\/fonts\/Inter-Regular\.ttf"\)/);assert.match(graphics,/sixflFont\(siteUrl, "\/fonts\/Inter-Bold\.ttf"\)/);assert.doesNotMatch(graphics,/node:fs|readFile\(|process\.cwd\(\)/);assert.doesNotMatch(graphics,/>SIXFL TV<\/text>/);assert.doesNotMatch(graphics,/>SIXFL PREDICTOR/);assert.match(graphics,/RECENT FORM/);assert.match(graphics,/PRE-MATCH PREDICTION/);assert.match(graphics,/typeLabel = isHighlights \? "HIGHLIGHTS" : "FULL MATCH"/);assert.match(graphics,/badgeGlow/);assert.match(graphics,/scoreShadow/);assert.match(graphics,/scorerLinesForTeam/);assert.match(graphics,/broadcastCodeForTeam/);assert.doesNotMatch(graphics,/GOALSCORERS/);assert.match(graphics,/monthUpper/);assert.match(graphics,/Nominate until 5/);assert.match(graphics,/winner announced from 13/);assert.match(graphics,/width="690" height="94"/);assert.match(graphics,/LEAGUE TABLE/);assert.match(graphics,/TOP HALF/);assert.match(graphics,/BOTTOM HALF/);assert.match(graphics,/teamNameY = predictor \? 470 : 330/);
