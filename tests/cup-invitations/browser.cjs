...
(async()=>{
 ...
 async function preview(_,form){window.calls.push('preview');const ids=JSON.parse(form.get('teamIds'));return {kind:form.get('kind'),teamIds:ids,preview:{previewKey:'sample',templateFingerprint:'sample',version:1,teams:teams.filter(t=>ids.includes(t.id)).map(t=>({teamId:t.id,teamName:t.teamName,error:null,contacts:[],previews:[{email:'a-long-captain-address@example.invalid',subject:'Cup invitation',bodyText:'£40 per match',bodyHtml:'<p>£40 per match. Rawdon and Catterick — proposed. A different night may apply.</p>'}]}))}}}
