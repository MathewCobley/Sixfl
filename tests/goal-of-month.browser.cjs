...
let browser,bundle;
const goal={id:'goal-one',fixtureId:'fixture-one',teamId:'team-one',monthKey:'2026-09',goalNumber:null,clipAssetId:'clip-one',clipNumber:1,clipUrl:'/api/goal-of-month/clips/goal-one',thumbnailUrl:'/api/goal-of-month/clips/goal-one/thumbnail',scorerName:'Test Scorer',teamName:'Example FC',opponentName:'Opponent FC',teamLogoUrl:null,leagueName:'Example League',kickoffAt:'2026-09-03T19:00:00Z',nominationCount:1,voteCount:0,videoUrls:[]};
function payload({nominees=true,eligible=true,voting=false}={}) {
  return {viewer:{signedIn:eligible,eligible},nominations:[{key:'2026-09',label:'September 2026',closesAt:'2026-10-05T23:00:00Z',usedNominations:0,maxNominations:3,nominatedCandidateIds:[],candidates:nominees?[goal]:[],fixtures:[{id:'fixture-one',kickoffAt:goal.kickoffAt,homeTeamId:'team-one',awayTeamId:'team-two',homeTeamName:'Example FC',awayTeamName:'Opponent FC',homeScore:6,awayScore:4,sixflTvUrl:'',videoUrls:[],leagueName:'Example League',clips:[{assetId:'clip-one',fixtureId:'fixture-one',clipNumber:1,filename:'goal.mp4',clipUrl:'/api/goal-of-month/fixtures/fixture-one/clips/clip-one',thumbnailUrl:'/api/goal-of-month/fixtures/fixture-one/clips/clip-one/thumbnail'}]}]}],voting:{key:'2026-09',label:'September 2026',open:voting,closesAt:'2026-10-12T23:00:00Z',candidates:voting?[goal]:[],selectedCandidateId:null},winners:[],legacy:{nominationsOpen:false,votingMayBeOpen:false,nominationsCloseAt:'2026-09-13T23:00:00Z',votingClosesAt:'2026-09-15T17:00:00Z'}};
}
test.before(async()=>{
...
async function screen(width,data,kind='panel') {
  ...
  await page.evaluate(({data,goal})=>{
    ...
    window.finishSave=body=>{
      ...
      else{const period=window.goalData.nominations[0];period.candidates=[{...window.testGoal,clipAssetId:body.clipAssetId,scorerName:body.scorerName}];period.nominatedCandidateIds=['goal-one'];period.usedNominations=1;}
...
for(const width of [390,1440]) {
  test(`monthly nomination save and shared dashboard footage at ${width}px`,async()=>{
    ...
    try{
      ...
      await page.getByLabel('Goal clip').selectOption('clip-one');
      await page.getByLabel('Scoring team').selectOption('team-one');
      await page.getByLabel('Scorer’s name').fill('Test Scorer');
      ...
      const clipVideo=page.locator('[data-monthly-goal="goal-one"] video');await clipVideo.waitFor();
      assert.equal(await clipVideo.getAttribute('src'),'/api/goal-of-month/clips/goal-one');
      assert.equal(await clipVideo.getAttribute('poster'),'/api/goal-of-month/clips/goal-one/thumbnail');
  ...
  test(`failed save keeps fixture selection and permits a deliberate retry at ${width}px`,async()=>{
    const {context,page}=await screen(width,payload({nominees:false}));
    try{
      await page.getByLabel('Recorded fixture').selectOption('fixture-one');await page.getByLabel('Goal clip').selectOption('clip-one');await page.getByLabel('Scoring team').selectOption('team-one');
      ...
      assert.equal(await page.getByLabel('Recorded fixture').inputValue(),'fixture-one');assert.equal(await page.getByLabel('Goal clip').inputValue(),'clip-one');assert.equal(await page.getByRole('button',{name:'Submit nomination',exact:true}).isEnabled(),true);
