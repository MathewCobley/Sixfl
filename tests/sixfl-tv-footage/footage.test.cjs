...
test('real private upload lifecycle, no live database or storage provider', async t => {
  ...
  try {
    ...
    const sql=fs.readFileSync('prisma/migrations/20260917170000_sixfl_tv_footage_uploads/migration.sql','utf8');
    ...
    await t.test('part retries are immutable and only complete manifests become previewable',async()=>{
      ...
      await assert.rejects(stream.streamFootage(new Request('https://sixfl.co.uk/test'),'match-a',asset.id),/not finished/);
