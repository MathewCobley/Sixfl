...
await build({stdin:{contents:`import React from 'react';import{createRoot}from'react-dom/client';import Uploader from './src/components/admin/sixfl-tv/FootageUploader';createRoot(document.getElementById('root')).render(<Uploader fixtureId="match-a" initial={window.__INITIAL__}/>);`,loader:'tsx',resolveDir:process.cwd()},bundle:true,outfile:path.join(out,'app.js'),platform:'browser',format:'iife',jsx:'automatic'});
...
const state=()=>({assets:assets.filter(a=>a.state!=='DELETED'),configured:true,partBytes,uploadedBytes:0,reservedBytes:assets.reduce((s,a)=>s+a.sizeBytes,0),limitBytes:100*1024**3});
const server=createServer(async(req,res)=>{
  ...
  if(url.pathname.startsWith('/api/')){
    assert.equal(url.pathname,'/api/admin/sixfl-tv/footage/match-a');
    if(req.method==='GET'){
    ...
    if(data.action==='begin'){
      const existing=assets.find(a=>a.state!=='DELETED'&&a.filename===data.filename&&a.kind===data.kind);
      if(existing)return send({asset:existing,reused:true});
      const asset={...data,id:'source-'+(assets.length+1),position:assets.length,partCount:Math.ceil(data.sizeBytes/partBytes),state:'UPLOADING',shared:false};delete asset.action;assets.push(asset);return send({asset,reused:false},201);
    }
  ...
  res.end(file?fs.readFileSync(path.join(out,file)):`<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><link rel="stylesheet" href="/app.css"><style>body{margin:0;background:#07140f;color:white;font-family:Arial}main{max-width:1150px;margin:20px auto;padding:16px;min-width:0}</style></head><body><main id="root"></main><script>window.__INITIAL__=${JSON.stringify(state()).replaceAll('<','\\u003c')}</script><script src="/app.js"></script></body></html>`);
});
